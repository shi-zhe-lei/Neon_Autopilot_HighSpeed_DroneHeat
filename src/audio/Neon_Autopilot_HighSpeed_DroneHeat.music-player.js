/*
 * Neon background-music library transport.
 *
...
(() => {
  ...
  if (!library) throw new Error('Neon music library dependency missing');
  const rhythm = window.NeonMusicRhythm;
  if (!rhythm) throw new Error('Neon music rhythm dependency missing');

  const MODE_STORAGE_KEY = 'cc-neon-music-mode';
  const TRACK_STORAGE_KEY = 'cc-neon-music-track';
  ...
  function createController(options = {}) {
    ...
    if (!storage && !Object.prototype.hasOwnProperty.call(options, 'storage')) {
      try {
        ...
        storageError = error;
        console.warn('[Neon music] Browser storage is unavailable; session choices remain available.', error);
      }
    }
    ...
    try {
      ...
      storageError = error;
      console.warn('[Neon music] Unable to read persisted library choices.', error);
    }
    ...
    function persist(key, value) {
      try {
        ...
        console.warn('[Neon music] Unable to persist a library choice.', error);
      }
    }
    ...
    function recordFailure(trackId, error) {
      ...
      console.warn(`[Neon music] Unable to play ${trackId}; the selected theme remains pending.`, error);
      emit();
      return false;
    }
    ...
    function onTrustedInteraction(event) {
      ...
      if (!window.NeonAudio?.shouldUnlockForInteraction?.(event)) return;
  ...
  window.NeonMusicPlayer = Object.freeze({
    version: 'Neon-music-player-8',
    MODE_STORAGE_KEY,
    TRACK_STORAGE_KEY,
