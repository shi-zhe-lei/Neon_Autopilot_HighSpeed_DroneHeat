 ...
 */
(() => {
  const STORAGE_KEY = 'cc-v23-audio-enabled';
  ...
  const EFFECT_ASSET_RELEASE_GENERATION = '23-ipad-road-residency-253';
  ...
  function createController(options = {}) {
    ...
    const inlineAssetModule = options.inlineAssetModule ?? window.NeonV23EffectAssets ?? null;
    ...
    if (!storage && !Object.prototype.hasOwnProperty.call(options, 'storage')) {
      try {
        ...
        storageError = error;
        console.warn('[V23 audio] Browser storage is unavailable; the session preference will still work.', error);
      }
    }
    ...
    try {
      ...
      storageError = error;
      console.warn('[V23 audio] Unable to read the persisted audio preference.', error);
    }
    ...
    function persistPreference() {
      try {
        ...
        console.warn('[V23 audio] Unable to persist the audio preference.', error);
      }
    }
    ...
    function ensureAssets() {
      ...
      assetLoadPromise = Promise.all(assetEntries.map(async ([key, descriptor]) => {
        try {
          ...
          diagnostics.lastError = String(error.message || error);
          console.warn(`[V23 audio] Unable to load ${key}; gameplay continues and synthesized feedback remains available.`, error);
          flushDeferredEvents(key);
        }
      })).then(() => {
        if (disposed) return false;
    ...
    async function unlock(reason = 'manual', trusted = false) {
      ...
      unlockPromise = (async () => {
        ...
        try {
          ...
          if (disposed) return false;
          diagnostics.lastError = String(error.message || error);
          console.warn('[V23 audio] Browser audio unlock failed; the next trusted interaction will retry.', error);
        }
    ...
    function scheduleSuspend(reason = 'context-suspended') {
      ...
      suspendTimer = scheduler.setTimeout(async () => {
        ...
        try {
          ...
          diagnostics.lastError = String(error.message || error);
          console.warn('[V23 audio] Unable to suspend the audio context.', error);
        }
      }, 140);
    }
    ...
    function play(name) {
      ...
      if (!definition) throw new RangeError(`Unknown V23 audio event: ${name}`);
      if (disposed) return false;
    ...
    function onVisibilityChange() {
      ...
      if (enabled && context) {
        ...
        Promise.resolve()
          ...
          console.warn('[V23 audio] Visibility resume was blocked; the next trusted input will retry.', error);
          emit();
          });
      }
  ...
  window.NeonV23Audio = Object.freeze({
    version: 'V23-audio-13',
    STORAGE_KEY,
