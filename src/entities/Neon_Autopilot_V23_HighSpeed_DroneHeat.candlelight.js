/*
 * V23 original candlelight factory / V23 原创烛光实体工厂。
...
window.NeonV23Candlelight = (() => {
  ...
  function createFactory(options = {}) {
    ...
    const modeling = options.modeling || window.NeonV23Modeling;
    if (!THREE?.Group || !THREE?.Mesh || !THREE?.InstancedMesh) {
      throw new Error('NeonV23Candlelight.createFactory requires a complete THREE namespace.');
    }
    if (typeof modeling?.createRadialGeometry !== 'function'
      || typeof modeling?.analyzeTopology !== 'function') {
      throw new Error('NeonV23Candlelight.createFactory requires NeonV23Modeling geometry and topology helpers.');
    }
    ...
    const visualSeed = String(options.visualSeed ?? options.seed ?? 'v23-candlelight-visual');
    ...
    function instantiateCollectible(definition, poolOrdinal) {
      ...
      root.name = `V23.Candlelight.${definition.id}`;
      const core = markMain(
        new THREE.Mesh(template.geometries.core, materials.core),
        definition
      );
      core.name = 'V23.Candlelight.OpaqueCore';
      ...
      flame.name = 'V23.Candlelight.FlameEffect';
      ...
      halo.name = 'V23.Candlelight.HaloEffect';
      ...
      trail.name = 'V23.Candlelight.TrailEffect';
    ...
    function instantiateAmbientCluster(definition, poolOrdinal) {
      ...
      const count = definition.recipe.candleCount;
      const root = new THREE.Group();
      root.name = `V23.AmbientCandlelight.${definition.id}`;
      const bases = new THREE.InstancedMesh(template.geometries.base, materials.base, count);
      bases.name = 'V23.AmbientCandlelight.InstancedBases';
      ...
      flames.name = 'V23.AmbientCandlelight.InstancedFlames';
