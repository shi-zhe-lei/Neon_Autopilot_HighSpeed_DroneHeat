/*
 * V23 watertight ship factory / V23 闭合飞船工厂。
 * Main meshes are generated only through NeonV23Modeling; effects are explicitly tagged and never join gameplay collision.
 */
window.NeonV23Ship = (() => {
  'use strict';
  ...
  const selectiveBloomLayer = Number.isInteger(window.NeonV23PostProcessing?.BLOOM_LAYER)
    ? window.NeonV23PostProcessing.BLOOM_LAYER
    : 12;
  const SHIP_ULTRA_PRESENTATION_CONTRACT = Object.freeze({
    qualityId: 'high',
    bloomLayer: selectiveBloomLayer,
    bloomMarker: 'neonV23Bloom',
  ...
  const TUNNEL_BOUNCE_CONTRACT = Object.freeze({
    version: 1,
    uniformName: 'neonV23ShipTunnelBounce',
    receiverGain: 0.72,
  ...
  function createShip({
    ...
    if (!window.NeonV23Modeling) throw new Error('V23 modeling module missing before ship factory');
    ...
    player.name = 'V23.WatertightShip';
    ...
    function installTunnelBounceReceiver(material) {
      if (!material?.isMeshStandardMaterial || material.transparent === true
        || Number(material.transmission) > 0 || material.userData?.neonV23?.kind === 'glass'
      ...
      material.onBeforeCompile = function onBeforeCompile(shader, renderer) {
        ...
        shader.fragmentShader = shader.fragmentShader
          .replace('#include <common>', '#include <common>\nuniform vec3 neonV23ShipTunnelBounce;')
          .replace('#include <lights_fragment_maps>', `#include <lights_fragment_maps>
#if defined(RE_IndirectDiffuse)
  irradiance += PI * neonV23ShipTunnelBounce * ${TUNNEL_BOUNCE_CONTRACT.receiverGain} / ${TUNNEL_BOUNCE_CONTRACT.referenceAlbedo};
#endif`);
      };
      ...
      material.userData.neonV23TunnelBounceReceiver = TUNNEL_BOUNCE_CONTRACT.version;
    ...
    function markMain(mesh, family, name) {
      ...
      const hasTransmissiveGlass = materials.some((material) => (
        material?.userData?.neonV23?.kind === 'glass'
    ...
    spiritCore.userData.neonV23Bloom = true;
    spiritCore.userData.neonV23BloomSource = 'ship-explicit-emitter';
    ...
    leftCapeGroup.name = 'V23.ShipCape.left';
    player.add(leftCapeGroup);
    const lifeFormGroups = [new THREE.Group(), new THREE.Group(), new THREE.Group()];
    lifeFormGroups[0].name = 'V23.ShipLife.healthy';
    lifeFormGroups[1].name = 'V23.ShipLife.wounded';
    lifeFormGroups[2].name = 'V23.ShipLife.critical';
    player.add(...lifeFormGroups);
    const rightCapeGroups = lifeFormGroups.map((lifeGroup, index) => {
      const capeGroup = new THREE.Group();
      capeGroup.name = `V23.ShipCape.right.${['healthy', 'wounded', 'critical'][index]}`;
    ...
    const shieldFieldMaterial = new THREE.ShaderMaterial({
      name: 'V23.DawnStarwardFieldMaterial',
    ...
    shieldFieldMaterial.userData.neonV23 = {
      kind: 'effect',
    ...
    shieldAnchors.name = 'V23.ShipEffect.shield-anchor-array';
    ...
    damageGroup.name = 'V23.Ship.DamageEffects';
    ...
    ultraContactTexture.name = 'V23.Ship.UltraContactShadow';
    ...
    ultraPlayerShadowMaterial.name = 'V23.Ship.UltraContactShadowMaterial';
    ...
    ultraPlayerShadowMaterial.customProgramCacheKey = () => 'v23-ultra-contact-shadow-v1';
    ...
    function getRenderQualityDiagnostics() {
      ...
      player.traverse((object) => {
        if (object.userData.neonV23Bloom === true) bloomEmitterCount++;
      });
      return Object.freeze({
