...
window.NeonWorld = (() => {
  ...
  function resolveLocalRainShelter(routeFrame = {}, out = null) {
    const target = out || {};
    const weatherClassifier = window.NeonWeather?.isSealedTunnelFrame;
  ...
  const configuredZones = window.NeonConfig?.zones;
  ...
  function createDecorationLayer({
    ...
    if (!Modeling) throw new Error('Neon modeling module missing before world module initialization');
    const RegionScenery = window.NeonRegionScenery;
    const Creatures = window.NeonCreatures;
    const Candlelight = window.NeonCandlelight;
    if (!RegionScenery) throw new Error('Neon region-scenery module missing before world module initialization');
    if (!Creatures) throw new Error('Neon creature module missing before world module initialization');
    if (!Candlelight) throw new Error('Neon candlelight module missing before world module initialization');
    const isSealedTunnelFrame = window.NeonWeather?.isSealedTunnelFrame;
    if (typeof isSealedTunnelFrame !== 'function') {
      throw new Error('Neon world requires the shared tunnel weather-shelter contract');
    }

    const zones = providedZones || window.NeonConfig?.zones || [];
    if (zones.length < familyManifest.length) throw new Error('Neon world requires six configured zones');
    ...
    if (RegionScenery.REGION_SCENERY_CONTRACT?.version !== REGION_SCENERY_INTEGRATION_CONTRACT.version
      ...
      throw new Error('Neon world requires the frozen 48-family region-scenery V2 contract.');
    }
    ...
    if (creatureTransitionContract?.policy !== CREATURE_ADJACENT_PREVIEW_POLICY
      ...
      throw new Error('Neon creature preview must share the configured world transition length and segmentation.');
    }
    if (typeof isReducedMotionEnabled !== 'function') {
      throw new TypeError('Neon world requires isReducedMotionEnabled to be a read-only callback.');
    }
    if (typeof sampleCommittedWorldFrameAtDistance !== 'function'
      ...
      throw new TypeError(
        'Neon classified scenery and creatures require committed-frame, rendered-terrain, and complete-road clearance authorities.'
      );
    }
    ...
    const seedInput = visualSeed ?? search.get('seed') ?? 'neon-world';
    if (typeof seedInput !== 'string') {
      throw new TypeError('Neon world visualSeed must be a string.');
    }
    const seed = seedInput;
    if (!seed.trim()) throw new TypeError('Neon world requires a non-empty string visual seed.');
    ...
    const randomUnit = () => {
      ...
      throw new Error('NeonModeling.createRng returned an unsupported generator');
    };
    ...
    const skyRandomUnit = () => {
      ...
      throw new Error('NeonModeling.createRng returned an unsupported sky generator');
    };
    ...
    const weatherRandomUnit = () => {
      ...
      throw new Error('NeonModeling.createRng returned an unsupported weather generator');
    };
    ...
    function installUltraWorldSurface(material) {
      if (!material || material.userData.neonUltraWorldDetail) {
        return material?.userData?.neonUltraWorldDetail || null;
      }
      const detailUniform = { value: 0 };
      const previousCompile = material.onBeforeCompile;
      const previousCacheKey = material.customProgramCacheKey?.bind(material);
      material.userData.neonUltraWorldDetail = detailUniform;
      material.onBeforeCompile = (shader, renderer) => {
        if (typeof previousCompile === 'function') previousCompile.call(material, shader, renderer);
        shader.uniforms.uNeonUltraWorldDetail = detailUniform;
        shader.vertexShader = shader.vertexShader
          .replace(
            '#include <common>',
            '#include <common>\nvarying vec3 vNeonUltraWorldPosition;'
          )
          .replace(
            '#include <begin_vertex>',
            '#include <begin_vertex>\nvNeonUltraWorldPosition = (modelMatrix * vec4(transformed, 1.0)).xyz;'
          );
        shader.fragmentShader = shader.fragmentShader
          .replace(
            '#include <common>',
            `#include <common>
uniform float uNeonUltraWorldDetail;
varying vec3 vNeonUltraWorldPosition;
float neonUltraHash(vec2 point) {
  return fract(sin(dot(point, vec2(127.1, 311.7))) * 43758.5453);
          ...
          .replace(
            ...
            `#include <color_fragment>
if (uNeonUltraWorldDetail > 0.5) {
  float neonMacro = sin(vNeonUltraWorldPosition.x * 0.018 + vNeonUltraWorldPosition.z * 0.013) * 0.5
    + sin(vNeonUltraWorldPosition.x * 0.071 - vNeonUltraWorldPosition.z * 0.063) * 0.25;
  float neonGrain = neonUltraHash(floor(vNeonUltraWorldPosition.xz * 3.2)) - 0.5;
  diffuseColor.rgb *= 1.0 + neonMacro * 0.105 + neonGrain * 0.075;
}`
          )
          .replace(
            ...
            `#include <roughnessmap_fragment>
if (uNeonUltraWorldDetail > 0.5) {
  float neonRoughMacro = sin(vNeonUltraWorldPosition.x * 0.043 + vNeonUltraWorldPosition.z * 0.051);
  float neonRoughGrain = neonUltraHash(floor(vNeonUltraWorldPosition.xz * 2.4)) - 0.5;
  roughnessFactor = clamp(roughnessFactor + neonRoughMacro * 0.065 + neonRoughGrain * 0.090, 0.18, 1.0);
}`
          )
          .replace(
            ...
            `#include <normal_fragment_maps>
if (uNeonUltraWorldDetail > 0.5) {
  float neonNormalX = sin(vNeonUltraWorldPosition.x * 2.7 + vNeonUltraWorldPosition.z * 1.9);
  float neonNormalZ = cos(vNeonUltraWorldPosition.z * 2.3 - vNeonUltraWorldPosition.x * 1.7);
  normal = normalize(normal + mat3(viewMatrix) * vec3(neonNormalX, 0.0, neonNormalZ) * 0.028);
}`
          );
      };
      material.customProgramCacheKey = () => `${previousCacheKey ? previousCacheKey() : ''}|neon-ultra-world-v1`;
    ...
    function installUltraCloudDetail(material) {
      const detailUniform = { value: 0 };
      material.userData.neonUltraCloudDetail = detailUniform;
      material.onBeforeCompile = (shader) => {
        shader.uniforms.uNeonUltraCloudDetail = detailUniform;
        shader.vertexShader = shader.vertexShader
          .replace('#include <common>', '#include <common>\nvarying float vNeonUltraCloudLocalY;')
          .replace('#include <begin_vertex>', '#include <begin_vertex>\nvNeonUltraCloudLocalY = transformed.y;');
        shader.fragmentShader = shader.fragmentShader
          .replace(
            '#include <common>',
            `#include <common>
uniform float uNeonUltraCloudDetail;
varying float vNeonUltraCloudLocalY;`
          )
          .replace(
            '#include <color_fragment>',
            `#include <color_fragment>
if (uNeonUltraCloudDetail > 0.5) {
  float neonCloudUnderside = 1.0 - smoothstep(-0.35, 0.42, vNeonUltraCloudLocalY);
  float neonCloudDensity = sin(vNeonUltraCloudLocalY * 13.0) * 0.025;
  diffuseColor.rgb *= 1.0 - neonCloudUnderside * 0.24 + neonCloudDensity;
}`
          )
          .replace(
            '#include <roughnessmap_fragment>',
            `#include <roughnessmap_fragment>
if (uNeonUltraCloudDetail > 0.5) roughnessFactor = clamp(roughnessFactor + 0.035, 0.0, 1.0);`
          );
      };
      material.customProgramCacheKey = () => 'neon-ultra-cloud-v1';
    ...
    function allowsOpaqueShadow(mesh) {
      ...
      return materials.every((material) => {
        const kind = material?.userData?.neon?.kind;
        return material
    ...
    function buildRegionPartGeometries(part, context) {
      ...
      throw new RangeError(`Unknown Neon region render part: ${part.kind}`);
    }
    ...
    function instantiateRegionTemplate({ definition, renderPlan, context }) {
      ...
      root.name = `Neon.RegionScenery.${definition.id}`;
      let triangles = 0;
    ...
    if (zoneTemplates.flat().length !== BASELINE_SCENERY_PRESENTATION_CONTRACT.familyCount) {
      throw new Error('Neon baseline scenery requires all 30 finished 3+2 realm families.');
    }
    ...
    if (cloudClusterCount !== CLOUD_WEATHER_LAYER_CONTRACT.clusterCount[mobile ? 'mobile' : 'desktop']) {
      throw new Error('Neon cloud lattice drifted from the monumental three-layer pool contract.');
    }
    ...
    const cloudLayers = CLOUD_WEATHER_LAYER_CONTRACT.layerIds.map((layerId, layerIndex) => {
      ...
      mesh.name = `Neon.Sky.CloudLayer.${layerId}`;
    ...
    const ultraSkyMaterial = new THREE.ShaderMaterial({
      name: 'Neon.World.UltraSkyGradientMaterial',
      uniforms: ultraSkyUniforms,
      vertexShader: `
varying vec3 vNeonSkyDirection;
void main() {
  vNeonSkyDirection = normalize(position);
...
varying vec3 vNeonSkyDirection;
void main() {
  vec3 direction = normalize(vNeonSkyDirection);
  ...
  gl_FragColor = vec4(color, 1.0);
    ...
    ultraSkyDome.name = 'Neon.World.UltraSkyGradientDome';
    ...
    function createUltraHorizonReliefGeometry(zoneIndex, farScenery) {
      ...
      geometry.userData.neonHorizon = Object.freeze({
        seed: profile.seed,
    ...
    function createUltraHorizonMaterial() {
      ...
      material.userData.neonHorizonColorBlend = colorBlendUniform;
      material.onBeforeCompile = (shader) => {
        ...
        if (!shader.vertexShader.includes(colorParsToken)
          ...
          throw new Error('Neon Ultra horizon shader is missing the required colour chunks');
        }
        shader.uniforms.neonHorizonColorBlend = colorBlendUniform;
        shader.vertexShader = shader.vertexShader
          .replace(
            colorParsToken,
            `${colorParsToken}
...
vColor = mix(
  vColor,
  neonNextHorizonColor,
  clamp(neonHorizonColorBlend, 0.0, 1.0)
);
#endif`
          );
      };
      material.customProgramCacheKey = () => 'neon-ultra-horizon-color-transition-v1';
      return material;
    }
    ...
    function createUltraHorizonGroup(zoneIndex) {
      ...
      group.name = `Neon.World.UltraHorizon.${zoneIndex}`;
      group.userData.neonFarScenery = farScenery;
      group.userData.neon = {
        modelClass: 'effect',
      ...
      relief.name = `Neon.World.UltraHorizonRelief.${zoneIndex}`;
    ...
    for (let zoneIndex = 0; zoneIndex < ultraHorizonGroups.length; zoneIndex++) {
      ...
      if (!currentRelief || !nextRelief) {
        throw new Error(`Neon Ultra horizon morph topology is missing realm ${zoneIndex}`);
      }
      for (const attributeName of ['position', 'normal', 'color']) {
        ...
        if (!currentAttribute || !nextAttribute || currentAttribute.count !== nextAttribute.count) {
          throw new Error(
            `Neon Ultra horizon ${attributeName} topology differs at realm ${zoneIndex}`
          );
        }
        if (attributeName === 'color') {
          currentRelief.geometry.setAttribute('neonNextHorizonColor', nextAttribute);
        } else {
      ...
      currentRelief.userData.neonNextHorizonZoneIndex =
        (zoneIndex + 1) % ultraHorizonGroups.length;
    }
    ...
    function auditUltraHorizonPresentation() {
      ...
      for (const group of ultraHorizonGroups) {
        group.traverse((object) => {
          ...
          for (const layer of object.geometry?.userData?.neonHorizon?.layerRanges || []) {
    ...
    function updateUltraEnvironment(
      ...
      for (let zoneIndex = 0; zoneIndex < ultraHorizonGroups.length; zoneIndex++) {
        ...
        const colorBlendUniform = relief?.material?.userData?.neonHorizonColorBlend;
    ...
    if (!moonSurfaceContext) {
      throw new Error('Neon sky requires a two-dimensional canvas context for the candle-moon surface');
    }
    ...
    if (!moonHaloContext) {
      throw new Error('Neon sky requires a two-dimensional canvas context for the candle-moon halo');
    }
    ...
    moonStarPetals.name = 'Neon.Sky.MemoryStarPetals';
    ...
    moonCloudVeils.name = 'Neon.Sky.MemoryCloudVeils';
    ...
    moonAnchor.name = 'Neon.Sky.MoonAnchor';
    moonAnchor.userData.neon = {
      modelClass: 'effect',
      collisionIndependent: true,
    ...
    moonHalo.name = 'Neon.Sky.CandleMoonHalo';
    ...
    moonDisc.name = 'Neon.Sky.CandleMoonDisc';
    ...
    moonVisual.name = 'Neon.Sky.CandleMoonMemoryLayers';
    ...
    weatherVolume.name = 'Neon.Weather.LocalVolume';
    ...
    if (!weatherParticleContext) {
      throw new Error('Neon weather particles require a two-dimensional canvas context');
    }
    ...
    if (!snowflakeContext) {
      throw new Error('Neon snowflakes require a two-dimensional canvas context');
    }
    ...
    rainMaterial.userData.neonLocalRainShelter = rainShelterUniforms;
    rainMaterial.onBeforeCompile = (shader) => {
      shader.uniforms.uNeonRainLocalExposure = rainShelterUniforms.localExposure;
      shader.uniforms.uNeonRainDistantExposure = rainShelterUniforms.distantExposure;
      shader.uniforms.uNeonRainShelterInnerM = rainShelterUniforms.innerRadiusM;
      shader.uniforms.uNeonRainShelterOuterM = rainShelterUniforms.outerRadiusM;
      shader.vertexShader = shader.vertexShader
        .replace(
          '#include <common>',
          '#include <common>\nvarying vec2 vNeonRainLocalXZ;'
        )
        .replace(
          '#include <begin_vertex>',
          '#include <begin_vertex>\nvNeonRainLocalXZ = transformed.xz;'
        );
      shader.fragmentShader = shader.fragmentShader
        .replace(
          '#include <common>',
          `#include <common>
uniform float uNeonRainLocalExposure;
uniform float uNeonRainDistantExposure;
uniform float uNeonRainShelterInnerM;
uniform float uNeonRainShelterOuterM;
varying vec2 vNeonRainLocalXZ;`
        )
        .replace(
          '#include <color_fragment>',
          `#include <color_fragment>
float neonRainExteriorBlend = smoothstep(
  uNeonRainShelterInnerM,
  uNeonRainShelterOuterM,
  length(vNeonRainLocalXZ)
);
diffuseColor.a *= mix(uNeonRainLocalExposure, uNeonRainDistantExposure, neonRainExteriorBlend);`
        );
    };
    rainMaterial.customProgramCacheKey = () => 'neon-local-rain-shelter-v1';
    const rainPool = new THREE.LineSegments(rainGeometry, rainMaterial);
    rainPool.name = 'Neon.Weather.Rain';
    ...
    function sampleRenderedTerrainHeight(worldX, worldZ) {
      ...
      if (!Number.isFinite(terrainY)) {
        throw new RangeError(
          `Neon terrain height authority returned a non-finite value at ${worldX}, ${worldZ}.`
        );
      }
    ...
    if (modelDebug) {
      ...
      console.info('[Neon modelDebug] world topology', {
        quality: qualityProfile.id,
        checked: topology.length,
        invalid: invalid.length,
        familyCounts: familyManifest.map((entry) => entry.landmarks.length + entry.environments.length)
      });
      if (invalid.length) console.warn('[Neon modelDebug] invalid world geometries', invalid);
    }
