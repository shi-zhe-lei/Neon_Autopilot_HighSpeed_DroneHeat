...
window.NeonV23Weather = (() => {
  ...
  function hashSeed(value) {
    const source = String(value ?? 'v23-weather');
  ...
  function validateContracts(zoneCount) {
    if (zoneCount !== zonePatterns.length) {
      throw new RangeError(`V23 weather requires ${zonePatterns.length} route realms; received ${zoneCount}.`);
    }
