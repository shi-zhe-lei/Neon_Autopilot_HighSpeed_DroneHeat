/*
 * V23 surface-weather interaction authority.
...
window.NeonV23SurfaceWeather = (() => {
  ...
  function createFlatBottomSnowBankGeometry(THREE, radialSegments, heightSegments) {
    ...
    geometry.computeBoundingSphere();
    geometry.userData.neonV23 = Object.freeze({
  ...
  function createPuddleSurfaceTextures(THREE, mobile) {
    ...
    if (!alphaContext) throw new Error('V23 puddle optics requires a two-dimensional canvas context');
    ...
    if (!roughnessContext) throw new Error('V23 puddle roughness requires a two-dimensional canvas context');
    ...
    if (!normalContext) throw new Error('V23 puddle normal map requires a two-dimensional canvas context');
  ...
  diffuseColor.a *= clamp(max(max(vColor.r, vColor.g), vColor.b), 0.0, 1.0);
    ...
    material.customProgramCacheKey = () => 'v23-instance-brightness-alpha-1';
    return material;
  }
  ...
  function createParticleTexture(THREE, kind) {
    ...
    if (!context) throw new Error('V23 surface weather requires a two-dimensional canvas context');
  ...
  function createParticlePool(THREE, root, capacity, kind, mobile) {
    ...
    object.name = `V23.SurfaceWeather.${kind === 'powder' ? 'SnowPowder' : 'WaterSpray'}`;
  ...
  function create(options = {}) {
    const {
      ...
      player,
      sweptEllipseInterval = window.NeonV23GameplayCore?.sweptEllipseInterval,
      surfaceFootprint = window.NeonV23Ship?.SURFACE_FOOTPRINT_CONTRACT,
      contactTarget = null,
    ...
    if (!THREE || !scene || !state || !player) {
      throw new TypeError('V23 surface weather requires THREE, scene, state, and player');
    }
    if (typeof sampleSignedFrame !== 'function' || typeof getWorldRenderOrigin !== 'function') {
      throw new TypeError('V23 surface weather requires route sampling and render-origin callbacks');
    }
    if (typeof sweptEllipseInterval !== 'function') {
      throw new TypeError('V23 surface weather requires the shared swept-ellipse contact solver');
    }
    ...
    if (!(footprintHalfWidthM > 0) || !(footprintHalfLengthM > 0)) {
      throw new TypeError('V23 surface weather requires the ship surface-footprint contract');
    }
    ...
    root.name = 'V23.SurfaceWeather.Root';
    ...
    puddles.name = 'V23.SurfaceWeather.Puddles';
    ...
    snowPatches.name = 'V23.SurfaceWeather.SnowPatches';
    ...
    snowBanks.name = 'V23.SurfaceWeather.SnowBanks';
    ...
    snowTracks.name = 'V23.SurfaceWeather.CompressedTracks';
    ...
    snowRidges.name = 'V23.SurfaceWeather.DisplacedSnowRidges';
    ...
    ripples.name = 'V23.SurfaceWeather.PuddleRipples';
    ...
    waterWakes.name = 'V23.SurfaceWeather.WaterWakes';
