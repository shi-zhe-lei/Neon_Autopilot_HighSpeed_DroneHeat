...
const NeonV23LightingModule = (() => {
  ...
  function weatherProfileFrom(input) {
    ...
    if (typeof weather === 'string') {
      ...
      return root.NeonV23Weather?.types?.[weather]?.profile || {};
    }
  ...
  function createHdrEnvironmentTexture(THREE) {
    ...
    texture.name = 'V23.PhysicalLighting.HdrEnvironment';
  ...
  function createPhysicalLightingRig(options = {}) {
    ...
    const previousTunnelLightingUniforms = scene.userData.neonV23TunnelLightingUniforms;
    const hadPreviousTunnelLightingUniforms = Object.prototype.hasOwnProperty.call(
      scene.userData,
      'neonV23TunnelLightingUniforms'
    );
    ...
    group.name = 'V23.PhysicalLightingRig';
    ...
    ambient.name = 'V23.PhysicalLighting.Ambient';
    group.add(ambient);
    ...
    const sun = createDirectional('V23.PhysicalLighting.Sun', effectiveSunShadowMapSize);
    const moon = createDirectional(
      'V23.PhysicalLighting.Moon',
      effectiveSecondaryDirectionalShadowMapSize
    );
    const lightning = createDirectional(
      'V23.PhysicalLighting.Lightning',
      effectiveSecondaryDirectionalShadowMapSize
    );
    ...
    const tunnelSpots = Array.from(
      ...
      (_, index) => {
        ...
        light.name = `V23.PhysicalLighting.Tunnel.${index}`;
        const target = new THREE.Object3D();
    ...
    fallbackAmbient.name = 'V23.PhysicalLighting.FlatVisibilityFallback';
    fallbackAmbient.visible = false;
    fallbackAmbient.userData.neonV23 = {
      modelClass: 'effect',
    ...
    function updateTunnelLightingUniforms() {
      ...
      tunnelLightingUniforms.neonV23GlobalAmbientTransmission.value = lightingEnabled
        ? ratio(ambient.intensity, outdoorAmbient)
        : 1;
      tunnelLightingUniforms.neonV23GlobalEnvironmentTransmission.value = lightingEnabled
      ...
      tunnelLightingUniforms.neonV23GlobalDirectionalTransmission.value = lightingEnabled
        ? Math.min(
    ...
    function dispose() {
      ...
      environmentTexture.dispose();
      if (scene.userData.neonV23TunnelLightingUniforms === tunnelLightingUniforms) {
        if (hadPreviousTunnelLightingUniforms) {
          scene.userData.neonV23TunnelLightingUniforms = previousTunnelLightingUniforms;
        } else {
          delete scene.userData.neonV23TunnelLightingUniforms;
        }
      }
    ...
    applyShadowPresentation();
    scene.userData.neonV23TunnelLightingUniforms = tunnelLightingUniforms;
...
if (typeof window === 'object') window.NeonV23Lighting = NeonV23LightingModule;
if (typeof module === 'object' && module.exports) module.exports = NeonV23LightingModule;
