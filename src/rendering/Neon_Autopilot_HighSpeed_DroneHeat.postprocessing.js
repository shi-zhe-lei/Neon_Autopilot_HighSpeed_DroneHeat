...
const NeonPostProcessingModule = (() => {
  ...
  function createCompositeFragmentShader(usesFxaa) {
    ...
    if (!COMPOSITE_FRAGMENT_SHADER.includes(uniformDeclaration)
      ...
      throw new Error('Neon composite anti-aliasing resolve marker is missing.');
    }
  ...
  function tagBloom(object, enabled = true) {
    ...
    object.userData.neonBloom = Boolean(enabled);
    return object;
  }
  ...
  function create({ THREE, renderer, scene, camera, auditLegacyPipeline = false } = {}) {
    ...
    function createTargetSet(targetWidth, targetHeight) {
      ...
      try {
        ...
        configureTexture(sceneTarget.texture, 'Neon High linear scene');
        ...
        sceneTarget.depthTexture.name = 'Neon High scene depth';
        ...
        const aoRawTarget = createColorTarget(halfWidth, halfHeight, 'Neon High AO raw');
        const aoPingTarget = createColorTarget(halfWidth, halfHeight, 'Neon High AO bilateral horizontal');
        const aoFinalTarget = createColorTarget(halfWidth, halfHeight, 'Neon High AO bilateral vertical');
        const bloomRawTarget = createColorTarget(halfWidth, halfHeight, 'Neon High Bloom selection', hdrType);
        const bloomPingTarget = createColorTarget(halfWidth, halfHeight, 'Neon High Bloom horizontal', hdrType);
        const bloomFinalTarget = createColorTarget(halfWidth, halfHeight, 'Neon High Bloom vertical', hdrType);
        allocated.push(
          aoRawTarget,
          aoPingTarget,
          aoFinalTarget,
...
if (typeof window === 'object') window.NeonPostProcessing = NeonPostProcessingModule;
if (typeof module === 'object' && module.exports) module.exports = NeonPostProcessingModule;
