/* Shared Neon procedural modeling kernel. Main visuals must be built from these closed indexed meshes. */
window.NeonModeling = (() => {
  'use strict';
  ...
  function requireThree(provided) {
    ...
    if (!THREE) throw new Error('NeonModeling requires THREE');
    return THREE;
  }
  ...
  function createLoftGeometry({ THREE: providedThree, rings, capStart = true, capEnd = true } = {}) {
    ...
    geometry.userData.neon.factory = 'createLoftGeometry';
    return geometry;
  }
  ...
  function createExtrudedProfileGeometry({
    ...
    geometry.userData.neon.factory = 'createExtrudedProfileGeometry';
    return geometry;
  }
  ...
  function createBeveledPrismGeometry({
    ...
    geometry.userData.neon.factory = 'createBeveledPrismGeometry';
    return geometry;
  }
  ...
  function createTubeGeometry({
    ...
    geometry.userData.neon.factory = 'createTubeGeometry';
    return geometry;
  }
  ...
  function createRadialGeometry({
    ...
    geometry.userData.neon.factory = 'createRadialGeometry';
    return geometry;
  }
  ...
  function createNoiseRockGeometry({
    ...
    const geometry = buildRingShell(THREE, rings, { capStart: true, capEnd: true });
    geometry.userData.neon.factory = 'createNoiseRockGeometry';
    geometry.userData.neon.seed = hashSeed(seed);
    return geometry;
  }
  ...
  function installProceduralSurface(material, { scale = 1.35, strength = 0.12, seed = 1 } = {}) {
    material.onBeforeCompile = (shader) => {
      shader.uniforms.neonPatternScale = { value: scale };
      shader.uniforms.neonPatternStrength = { value: strength };
      shader.uniforms.neonPatternSeed = { value: (hashSeed(seed) % 10_000) / 10_000 };
      shader.vertexShader = shader.vertexShader
        .replace('#include <common>', '#include <common>\nvarying vec3 vNeonLocalPosition;')
        .replace('#include <begin_vertex>', '#include <begin_vertex>\nvNeonLocalPosition = transformed;');
      shader.fragmentShader = shader.fragmentShader
        .replace('#include <common>', `#include <common>
varying vec3 vNeonLocalPosition;
uniform float neonPatternScale;
uniform float neonPatternStrength;
uniform float neonPatternSeed;
float neonHash(vec3 p) {
  p = fract(p * 0.1031);
  p += dot(p, p.yzx + 33.33 + neonPatternSeed);
...
float neonDiffusePattern = neonHash(floor(vNeonLocalPosition * neonPatternScale));
diffuseColor.rgb *= 1.0 + (neonDiffusePattern - 0.5) * neonPatternStrength;`)
        .replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>
float neonRoughnessPattern = neonHash(floor(
  vNeonLocalPosition * neonPatternScale + vec3(11.7, 29.3, 47.1)
));
roughnessFactor = clamp(
  roughnessFactor + (0.5 - neonRoughnessPattern) * neonPatternStrength * 0.42,
  0.04,
  1.0
);`);
    };
    material.customProgramCacheKey = () => `neon-procedural-linear-v2-${scale}-${strength}-${hashSeed(seed)}`;
  }
  ...
  function resolveQualityProfile(requested = 'auto', {
    ...
    profiles = window.NeonConfig?.qualityProfiles || DEFAULT_QUALITY_PROFILES
  } = {}) {
  ...
  function collectMeshTopology(object, tolerance) {
    ...
    const visit = (node) => {
      if (node?.isMesh && node.geometry?.isBufferGeometry) {
        ...
        let report = null;
        if (geometry.userData?.neonImmutableTopology) {
