/* Equal-scale cloverleaf minimap renderer. Track topology and frames remain authoritative in NeonV23Track. */
window.NeonV23CloverleafMap = (() => {
  'use strict';
  ...
  function resolveTerrainPalette(options) {
    const configuredZones = collectionValues(options.zones || window.NeonV23Config?.zones);
    if (!configuredZones.length) {
      throw new TypeError('NeonV23CloverleafMap.create requires configured terrain zones.');
    }
    return Object.freeze(configuredZones.map((zone, index) => {
      const color = terrainColorToCss(zone?.terrain?.base);
      if (!color) {
        throw new TypeError(`NeonV23CloverleafMap terrain zone ${index} requires terrain.base.`);
      }
      return color;
    }));
  }
  ...
  function createBufferCanvas(width, height, createCanvas) {
    ...
    throw new Error('NeonV23CloverleafMap requires OffscreenCanvas, document, or options.createCanvas.');
  }

  function recoverableMinimapError(message, options = {}) {
    const ErrorClass = window.NeonV23MinimapErrors?.NeonV23MinimapRecoverableError;
    ...
    error.name = 'NeonV23MinimapRecoverableError';
  ...
  function createAtomicPresenter(options = {}) {
    ...
    if (!primaryCanvas || typeof primaryCanvas.getContext !== 'function') {
      throw new TypeError('NeonV23CloverleafMap.createAtomicPresenter requires a primary canvas.');
    }
    if (!standbyCanvas || standbyCanvas === primaryCanvas || typeof standbyCanvas.getContext !== 'function') {
      throw new TypeError(
        'NeonV23CloverleafMap atomic presentation requires a distinct standby canvas.'
      );
    }
    const primaryContext = primaryCanvas.getContext('2d', { alpha: true });
    if (!primaryContext) {
      throw new Error('NeonV23CloverleafMap could not acquire its primary presentation context.');
    }
    const standbyContext = standbyCanvas.getContext('2d', { alpha: true });
    if (!standbyContext) {
      throw new Error('NeonV23CloverleafMap could not acquire its standby presentation context.');
    }
    const surfaces = [
    ...
    function present(sourceCanvas) {
      if (destroyed) {
        throw new Error('NeonV23CloverleafMap presenter is destroyed.');
      }
  ...
  function create(options = {}) {
    ...
    if (!canvas || typeof canvas.getContext !== 'function') {
      throw new TypeError('NeonV23CloverleafMap.create requires a canvas.');
    }
    ...
    const track = options.track || window.NeonV23Track;
    if (!track) throw new TypeError('NeonV23CloverleafMap.create requires the track API.');
    let graph = options.graph || track.graph;
    if (!graph) throw new TypeError('NeonV23CloverleafMap.create requires track.graph or options.graph.');
