/* Runtime tile-local visual pool for the directed Neon cloverleaf itinerary. */
window.NeonCloverleafTilePool = (() => {
  'use strict';
  ...
  function graphForTile(trackOrTile, tileOrPathPlan, pathPlanOrOptions, maybeOptions) {
    ...
    const track = explicitTrack ? trackOrTile : window.NeonTrack;
    ...
    if (!track?.graph || typeof track.getEdge !== 'function' || typeof track.sampleEdge !== 'function') {
      throw new Error('graphForTile requires the NeonTrack directed graph contract');
    }
    ...
    const graph = {
      version: `${track.graph.version || 'Neon-cloverleaf'}:${tile.token}`,
  ...
  function create(options) {
    ...
    const visualFactory = options?.visualFactory || window.NeonCloverleafVisuals;
    if (!track?.graph || typeof track.getEdge !== 'function' || typeof track.sampleEdge !== 'function') {
    ...
    if (!visualFactory || typeof visualFactory.create !== 'function') {
      throw new Error('Cloverleaf tile pool requires NeonCloverleafVisuals.create');
    }
