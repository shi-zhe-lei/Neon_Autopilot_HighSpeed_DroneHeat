/* Static full-cloverleaf road network visuals. Gameplay topology remains owned by NeonTrack. */
window.NeonCloverleafVisuals = (() => {
  'use strict';
  ...
  function createSupportGroundHeightResolver(options = {}, runtimeTrack = options.runtimeTrack || options.track) {
    ...
    const candidates = [
      ...
      [
        'scene.userData.neonTerrainHeightAt',
        sceneData,
        sceneData?.neonTerrainHeightAt
      ]
    ];
  ...
  function createRoadMarkingRibbonGeometry(THREE, lineGeometry, width = 0.19, lift = 0.006) {
    ...
    geometry.computeBoundingSphere();
    geometry.userData.neonUltraRoadMarking = true;
    return geometry;
  }
  ...
  function markSkyRouteGeometry(geometry, role, topology, localEnvelope = null) {
    geometry.userData.neonSkyRouteScenery = Object.freeze({
  ...
  function markSkyRouteMaterial(material, role) {
    material.userData.neonSkyRouteScenery = Object.freeze({
      contractVersion: SKY_ROUTE_SCENERY_CONTRACT.version,
      role,
  ...
  function injectStaticTunnelIrradianceShader(shader, THREE, tunnelLightingUniforms) {
    ...
    shader.vertexShader = shader.vertexShader
      .replace(
        '#include <common>',
        '#include <common>\nattribute float staticTunnelIrradiance;\nattribute float tunnelEnclosure;\nvarying float vNeonStaticTunnelIrradiance;\nvarying float vNeonTunnelEnclosure;'
      )
      .replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\nvNeonStaticTunnelIrradiance = staticTunnelIrradiance;\nvNeonTunnelEnclosure = tunnelEnclosure;'
      );
    ...
    const receivedLightChunk = THREE.ShaderChunk.lights_fragment_begin.replace(
      ...
      `getDirectionalLightInfo( directionalLight, directLight );
        directLight.color *= neonTunnelOutdoorTransmission(0.025, neonGlobalDirectionalTransmission);`
    );
...
varying float vNeonStaticTunnelIrradiance;
varying float vNeonTunnelEnclosure;
uniform float neonGlobalAmbientTransmission;
uniform float neonGlobalEnvironmentTransmission;
uniform float neonGlobalDirectionalTransmission;
float neonTunnelOutdoorTransmission(float deepTransmission, float globalTransmission) {
  return min(1.0, mix(1.0, deepTransmission, clamp(vNeonTunnelEnclosure, 0.0, 1.0)) / max(globalTransmission, 0.0001));
}`
      )
  ...
  irradiance *= neonTunnelOutdoorTransmission(0.14, neonGlobalAmbientTransmission);
  iblIrradiance *= neonTunnelOutdoorTransmission(0.14, neonGlobalEnvironmentTransmission);
  irradiance += PI * vec3(${red}, ${green}, ${blue})
    * max(vNeonStaticTunnelIrradiance, 0.0) / 0.18;
#endif
#if defined(RE_IndirectSpecular)
  radiance *= neonTunnelOutdoorTransmission(0.14, neonGlobalEnvironmentTransmission);
  clearcoatRadiance *= neonTunnelOutdoorTransmission(0.14, neonGlobalEnvironmentTransmission);
#endif`
      );
  }
  ...
  function installLowRoadShader(THREE, material, tunnelLightingUniforms) {
    ...
    material.customProgramCacheKey = () => 'neon-low-road-static-tunnel-irradiance-v3';
    material.userData.neonStaticTunnelIrradiance = STATIC_TUNNEL_IRRADIANCE_CONTRACT.version;
  }
  ...
  function installStaticTunnelStructureShader(THREE, material, signature, tunnelLightingUniforms) {
    material.onBeforeCompile = (shader) => injectStaticTunnelIrradianceShader(shader, THREE, tunnelLightingUniforms);
    material.customProgramCacheKey = () => `neon-static-tunnel-structure-${signature}-v3`;
    material.userData.neonStaticTunnelIrradiance = STATIC_TUNNEL_IRRADIANCE_CONTRACT.version;
  }
  ...
  function installStandardRoadShader(THREE, material, tunnelLightingUniforms) {
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader
        .replace('#include <common>', '#include <common>\nattribute vec2 roadCoord;\nvarying vec2 vNeonRoadCoord;')
        .replace('#include <begin_vertex>', '#include <begin_vertex>\nvNeonRoadCoord = roadCoord;');
      shader.fragmentShader = shader.fragmentShader
        .replace('#include <common>', `#include <common>
varying vec2 vNeonRoadCoord;
float neonStandardRoadHash(vec2 point) {
  vec3 value = fract(vec3(point.xyx) * vec3(0.1031, 0.11369, 0.13787));
  value += dot(value, value.yzx + 19.19);
  return fract((value.x + value.y) * value.z);
}
float neonStandardRoadJoint(vec2 roadCoord) {
  float distanceToJoint = abs(fract(roadCoord.y / 45.0) - 0.5) * 45.0;
  return 1.0 - smoothstep(0.07, 0.22, distanceToJoint);
}
float neonStandardTyreWear(float lateral) {
  return 1.0 - smoothstep(0.06, 0.15, abs(abs(lateral) - 0.38));
}`)
        .replace('#include <map_fragment>', `#include <map_fragment>
float neonStandardFade = 1.0 - smoothstep(55.0, 230.0, length(vViewPosition));
float neonStandardGrain = neonStandardRoadHash(
  floor(vNeonRoadCoord * vec2(12.0, 0.38))
);
float neonStandardJoint = neonStandardRoadJoint(vNeonRoadCoord);
float neonStandardTyre = neonStandardTyreWear(vNeonRoadCoord.x);
float neonStandardShoulder = smoothstep(0.70, 0.98, abs(vNeonRoadCoord.x));
diffuseColor.rgb *= vec3(0.92);
diffuseColor.rgb *= 1.0 + (neonStandardGrain - 0.5) * 0.018 * neonStandardFade;
diffuseColor.rgb *= 1.0 - neonStandardJoint * 0.065
  - neonStandardTyre * 0.032 * neonStandardFade - neonStandardShoulder * 0.055;`)
        .replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>
float neonStandardRoughness = neonStandardRoadHash(
  floor(vNeonRoadCoord * vec2(16.0, 0.54) + 5.0)
);
roughnessFactor = clamp(
  roughnessFactor + (neonStandardRoughness - 0.5) * 0.035 * neonStandardFade
    + neonStandardJoint * 0.035 + neonStandardShoulder * 0.045,
  0.30,
  1.0
);`);
      injectStaticTunnelIrradianceShader(shader, THREE, tunnelLightingUniforms);
    };
    material.customProgramCacheKey = () => 'neon-standard-road-coordinate-v4';
    material.userData.neonSurfaceDetail = 'standard-road-coordinate';
    material.userData.neonStaticTunnelIrradiance = STATIC_TUNNEL_IRRADIANCE_CONTRACT.version;
  }
  ...
  function installUltraRoadShader(THREE, material, tunnelLightingUniforms) {
    material.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        .replace('#include <common>', `#include <common>
varying vec2 vNeonRoadCoord;
float neonRoadHash(vec2 point) {
  vec3 value = fract(vec3(point.xyx) * vec3(0.1031, 0.11369, 0.13787));
  value += dot(value, value.yzx + 19.19);
  return fract((value.x + value.y) * value.z);
}
float neonRoadJoint(vec2 roadCoord) {
  float distanceToJoint = abs(fract(roadCoord.y / 45.0) - 0.5) * 45.0;
  return 1.0 - smoothstep(0.06, 0.18, distanceToJoint);
}
float neonTyreWear(float lateral) {
  return 1.0 - smoothstep(0.045, 0.13, abs(abs(lateral) - 0.38));
}`)
        .replace('#include <map_fragment>', `#include <map_fragment>
float neonRoadDetailFade = 1.0 - smoothstep(70.0, 280.0, length(vViewPosition));
float neonRoadGrain = neonRoadHash(floor(vNeonRoadCoord * vec2(24.0, 0.7)));
float neonRoadJointMask = neonRoadJoint(vNeonRoadCoord);
float neonRoadTyreMask = neonTyreWear(vNeonRoadCoord.x);
float neonRoadEdgeDirt = smoothstep(0.68, 0.98, abs(vNeonRoadCoord.x));
...
diffuseColor.rgb *= 1.0 + (neonRoadGrain - 0.5) * 0.02375 * neonRoadDetailFade;
diffuseColor.rgb *= 1.0 - neonRoadJointMask * 0.05
  - neonRoadTyreMask * 0.035 * neonRoadDetailFade - neonRoadEdgeDirt * 0.05;`)
        .replace('#include <roughnessmap_fragment>', `float neonRoadBaseRoughness = roughness;
#include <roughnessmap_fragment>
roughnessFactor = mix(neonRoadBaseRoughness, roughnessFactor, neonRoadDetailFade);
float neonUltraRoughness = neonRoadHash(floor(vNeonRoadCoord * vec2(32.0, 1.0) + 7.0));
roughnessFactor = clamp(
  roughnessFactor + (neonUltraRoughness - 0.5) * 0.054 * neonRoadDetailFade
    + neonRoadJointMask * 0.035
    + smoothstep(0.70, 0.98, abs(vNeonRoadCoord.x)) * 0.05,
  0.22,
  1.0
);`)
        .replace('#include <normal_fragment_maps>', `vec3 neonRoadGeometricNormal = normal;
#include <normal_fragment_maps>
normal = normalize(mix(neonRoadGeometricNormal, normal, neonRoadDetailFade));
float neonRoadMicroHeight = neonRoadHash(vNeonRoadCoord * vec2(45.75, 1.75));
vec3 neonRoadDx = dFdx(vec3(vNeonRoadCoord, neonRoadMicroHeight));
vec3 neonRoadDy = dFdy(vec3(vNeonRoadCoord, neonRoadMicroHeight));
vec3 neonRoadMicroCross = cross(neonRoadDx, neonRoadDy);
float neonRoadMicroLengthSquared = dot(neonRoadMicroCross, neonRoadMicroCross);
// Caps and skirts may collapse one texture axis; keep their geometric normal instead of normalizing zero.
float neonRoadMicroWeight = step(1.0e-10, neonRoadMicroLengthSquared);
vec2 neonRoadMicroSlope = neonRoadMicroCross.xy
  * inversesqrt(max(neonRoadMicroLengthSquared, 1.0e-10))
  * neonRoadMicroWeight;
normal = normalize(normal + vec3(neonRoadMicroSlope * 0.03 * neonRoadDetailFade, 0.0));`);
      injectStaticTunnelIrradianceShader(shader, THREE, tunnelLightingUniforms);
    };
    material.customProgramCacheKey = () => 'neon-ultra-road-coordinate-v6';
    material.userData.neonStaticTunnelIrradiance = STATIC_TUNNEL_IRRADIANCE_CONTRACT.version;
  }
  ...
  function installUltraStructureShader(
    ...
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader
        .replace('#include <common>', '#include <common>\nvarying vec3 vNeonStructurePosition;')
        .replace('#include <begin_vertex>', '#include <begin_vertex>\nvNeonStructurePosition = transformed;');
      shader.fragmentShader = shader.fragmentShader
        .replace('#include <common>', `#include <common>
varying vec3 vNeonStructurePosition;
float neonStructureHash(vec3 point) {
  point = fract(point * 0.1031);
  point += dot(point, point.yzx + 31.32);
  return fract((point.x + point.y) * point.z);
}`)
        .replace('#include <map_fragment>', `#include <map_fragment>
float neonStructureVariation = neonStructureHash(
  floor(vNeonStructurePosition * ${Number(scale).toFixed(4)})
);
diffuseColor.rgb *= 1.0 + (neonStructureVariation - 0.5) * ${Number(strength).toFixed(4)};`)
        .replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>
float neonStructureRoughness = neonStructureHash(
  floor(vNeonStructurePosition * ${Number(scale * 1.7).toFixed(4)} + 13.7)
);
roughnessFactor = clamp(
  roughnessFactor + (neonStructureRoughness - 0.5) * ${Number(strength * 0.8).toFixed(4)},
  0.08,
  1.0
);`);
    ...
    material.customProgramCacheKey = () => (
      `neon-ultra-structure-${signature}-${receiveStaticTunnelIrradiance ? 'tunnel-lit-v3' : 'v1'}`
    );
  ...
  function createIndexedGeometryMergeJob(THREE, geometries, boundingSphere, options = {}) {
    ...
    function roadCoordinateContractError(message) {
      const ErrorType = window.NeonErrors?.NeonRenderingError || Error;
      ...
      if (stage === 'warm-center') {
        ...
        if (!roadMergeJob) {
          ...
          Object.assign(roadShellGeometry.userData, {
            neonJumpPlatformGeometry: metrics.jumpPlatformCount > 0,
  ...
  function createTrafficGeometry(THREE, modeling) {
    ...
    geometry.userData.neonAmbientTrafficArt = Object.freeze({
    ...
    geometry.userData.neonTrafficShape = Object.freeze({
  ...
  function createSkyPetalSpireRibbonGeometry(THREE, modeling) {
    ...
    geometry.userData.neonSkyWayfindingArt = Object.freeze({
  ...
  function createTunnelShellGeometryAudit(options) {
    ...
    if (!THREE || !track?.graph || typeof track.sampleEdge !== 'function') {
      throw new Error('Tunnel shell geometry audit requires THREE and the NeonTrack graph sampler');
    }
  ...
  function acquireExitSignAtlas(THREE, specs, columns, rows, cellWidth, cellHeight) {
    ...
    if (!atlas) {
      ...
      atlas.material.userData.neonSkyWayfindingArt = Object.freeze({
        contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
  ...
  function createExitSignBatch(THREE, specs, tileOrigin, onPresentationTexturesChanged = null) {
    ...
    mesh.userData.atlasLanguage = window.NeonI18n?.language || 'zh-CN';
    mesh.userData.neonSkyWayfindingArt = Object.freeze({
      contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
    ...
    if (typeof window.NeonI18n?.subscribe === 'function') {
      unsubscribeLanguage = window.NeonI18n.subscribe(syncLanguage);
    }
  ...
  function* createOwnedVisualGenerator(options, own) {
    ...
    if (!THREE || !track?.graph || typeof track.sampleEdge !== 'function') {
      throw new Error('Cloverleaf visuals require THREE and the NeonTrack graph sampler');
    }
    ...
    const tunnelLightingUniforms = scene.userData?.neonTunnelLightingUniforms || {
    ...
    group.name = 'neon-full-cloverleaf-network';
    ...
    own(lowRoadMaterial);
    lowRoadMaterial.userData.neonSurfaceDetail = 'low-physical-without-coordinate-shader';
    ...
    ultraRoadMaterial.userData.neonUltraSurface = 'road';
    ...
    roadShell.geometry.userData.neonImmutableTopology = true;
    const sharedRoadVisualContract = roadShell.geometry.userData.neonSharedMainVisualContract;
    if (sharedRoadVisualContract) {
      ...
      roadShell.userData.neon = sharedRoadVisualContract.neon;
      ...
      roadShell.geometry.userData.neonSharedMainVisualContract = Object.freeze({
        modelRole: roadShell.userData.modelRole,
        modelLabel: roadShell.userData.modelLabel,
        neon: roadShell.userData.neon,
    ...
    underpassLights.userData.neonBloom = true;
    underpassLights.layers.enable(12);
    underpassLightMaterial.userData.neon = Object.freeze({
      bloomEmitter: true,
      kind: 'constant-on-candle-star-diffuser',
      contractVersion: SKY_ROUTE_SCENERY_CONTRACT.version
    });
    underpassLights.userData.neonSkyWayfindingArt = Object.freeze({
      contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
    ...
    structuralDetailMaterial.userData.neonSkyWayfindingArt = Object.freeze({
      contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
    ...
    ultraStructuralDetailMaterial.userData.neonSkyWayfindingArt =
      structuralDetailMaterial.userData.neonSkyWayfindingArt;
    ...
    function cameraBlockerLocalEnvelope(geometry) {
      const cached = geometry?.userData?.neonFilmCameraBlockerEnvelope;
      ...
      geometry.userData.neonFilmCameraBlockerEnvelope = envelope;
      return envelope;
    }
    ...
    goreGeometry.userData.neonSkyWayfindingArt = Object.freeze({
      contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
    ...
    own(goreMaterial);
    goreMaterial.userData.neonSkyWayfindingArt = Object.freeze({
      contractVersion: SKY_ROUTE_WAYFINDING_ART_CONTRACT.version,
    ...
    own(trafficGeometry);
    const trafficShape = trafficGeometry.userData.neonTrafficShape;
    ...
    own(trafficMaterial);
    trafficMaterial.userData.neonAmbientTrafficArt = Object.freeze({
      contractVersion: AMBIENT_TRAFFIC_ART_CONTRACT.version,
    ...
    return Object.freeze({
      ...
      diagnostics: Object.freeze({
        ...
        standardRoadCoordinateDetail: roadMaterial.userData.neonSurfaceDetail,
        lowRoadCoordinateDetail: lowRoadMaterial.userData.neonSurfaceDetail,
        ...
        decisionStructureGeometryRole:
          structuralDetailGeometry.userData.neonSkyWayfindingArt.role,
        decisionStructureGeometryTopology:
          structuralDetailGeometry.userData.neonSkyWayfindingArt.topology,
  ...
  function createBuildJob(options) {
    if (!options?.THREE || !options?.track?.graph || typeof options.track.sampleEdge !== 'function') {
      throw new Error('Cloverleaf build job requires THREE and the NeonTrack graph sampler');
    }
  ...
  function createGeometryBatchAuditJob(options) {
    if (!options?.THREE || !options?.track?.graph || typeof options.track.sampleEdge !== 'function') {
      throw new Error('Cloverleaf geometry audit requires THREE and the NeonTrack graph sampler');
    }
