...
const cloverleafSource = fs.readFileSync(
  path.join(PROJECT_ROOT, 'src/navigation/Neon_Autopilot_HighSpeed_DroneHeat.cloverleaf.js'),
  'utf8'
);
const lightingSource = fs.readFileSync(
  path.join(PROJECT_ROOT, 'src/rendering/Neon_Autopilot_HighSpeed_DroneHeat.lighting.js'),
  'utf8'
);
...
function evaluateRuntimeFallbackInterchangeArt() {
  ...
  return new Function(
    ...
    { NeonCloverleafVisuals: { skyRouteSceneryContract: contract } },
    THREE,
    globalThis.NeonModeling,
    0.65
  );
...
function createTerrainGeometryAuditFactory(contract, qualityProfileId) {
  ...
  return new Function(
    ...
    class NeonRenderingError extends Error {}
  );
}
...
test('launch construction profiles retain compatibility metadata while render tiers own live budgets', () => {
  assert.equal(globalThis.NeonConfig.qualityProfiles.high.shadowMapSize, 4_096);
  assert.equal(globalThis.NeonConfig.qualityProfiles.mobile.shadowMapSize, 2_048);
  ...
  const { renderQualityContract, renderQualityProfiles } = globalThis.NeonConfig;
...
test('non-emitting material kinds prefer physical BRDF defaults', () => {
  for (const kind of ['surface', 'structure', 'organic', 'road']) {
    ...
    assert.equal(material.userData.neon.physical, true);
  }
...
test('Sky terrain art v2 builds six grouped sealed scenes and six sealed ecotones without runtime batches', () => {
  ...
  const zones = globalThis.NeonConfig.zones;
  ...
  for (const qualityProfileId of ['high', 'mobile']) {
    ...
    for (let zoneIndex = 0; zoneIndex < zones.length; zoneIndex++) {
      ...
      for (const secondaryZone of [null, nextZone]) {
        ...
        const featureCluster = geometry.getAttribute('neonTerrainFeatureCluster');
...
test('High16 and Mobile10 terrain feature clusters are masked by every graph road capsule in all rotations', () => {
  ...
  assert.match(shaderSource, /attribute float neonTerrainFeatureCluster;/);
  assert.match(shaderSource, /attribute vec2 neonTerrainRoadFeatureMask;/);
  assert.match(shaderSource, /vNeonTerrainRoadFeatureVisible < 0\.5\) discard;/);
  assert.match(shaderSource, /uNeonTerrainTunnelCutoutCount/);
  const maskSource = extractFunctionDeclaration(mainSource, 'terrainRoadFeatureMaskForCell');
...
test('High terrain detail restores absolute world coordinates after floating-origin rebasing', () => {
  ...
  assert.match(terrainShader, /uniform vec2 uNeonTerrainWorldOrigin;/);
  assert.match(
    terrainShader,
  ...
  assert.doesNotMatch(
    terrainShader,
    /vNeonTerrainWorldPosition = \(modelMatrix \* neonTerrainInstancePosition\)\.xyz;/
  );
...
test('base and High terrain clip only the committed underground tunnel corridor', () => {
  ...
  assert.match(
    shaderSource,
    /neonTunnelSegment >= uNeonTerrainTunnelCutoutCount\) break;/
  );
  assert.match(
    shaderSource,
    /vNeonTerrainWorldPosition\.xz - neonTunnelStart\.xy/
  );
  assert.match(shaderSource, /float neonTunnelRadius = mix\(/);
  assert.match(shaderSource, /neonTunnelRadius > 0\.001[\s\S]*?\) discard;/);
...
test('runtime fallback interchange uses sealed Sky-route v2 stone without changing its fixed capacities', () => {
  ...
  assert.equal(contract.version, 2);
  assert.equal(bridgeSupportMat.userData.neonSkyRouteScenery.contractVersion, 2);
  ...
  assert.deepEqual(
    bridgePierGeo.userData.neonSkyRouteScenery,
    {
      contractVersion: 2,
  ...
  assert.equal(bridgeBeamGeo.userData.neonSkyRouteScenery.outlinePointCount, 16);
  assert.equal(bridgeBeamGeo.userData.neonSkyRouteScenery.axialRingCount, 6);
  for (const [label, geometry] of [
    ['pier', bridgePierGeo],
    ['beam', bridgeBeamGeo]
  ]) {
    const topology = globalThis.NeonModeling.analyzeTopology(geometry);
