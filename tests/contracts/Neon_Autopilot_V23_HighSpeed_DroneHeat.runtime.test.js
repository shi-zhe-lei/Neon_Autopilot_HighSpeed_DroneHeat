...
const weatherSource = readFileSync(
  new URL('src/weather/Neon_Autopilot_V23_HighSpeed_DroneHeat.weather.js', PROJECT_ROOT),
  'utf8'
);
const surfaceWeatherSource = readFileSync(
  new URL('src/weather/Neon_Autopilot_V23_HighSpeed_DroneHeat.surface-weather.js', PROJECT_ROOT),
  'utf8'
);
const lightingSource = readFileSync(
  new URL('src/rendering/Neon_Autopilot_V23_HighSpeed_DroneHeat.lighting.js', PROJECT_ROOT),
  'utf8'
);
const cloverleafSource = readFileSync(
  new URL('src/navigation/Neon_Autopilot_V23_HighSpeed_DroneHeat.cloverleaf.js', PROJECT_ROOT),
  'utf8'
);
const cloverleafTilePoolSource = readFileSync(
  new URL('src/navigation/Neon_Autopilot_V23_HighSpeed_DroneHeat.cloverleaf-tiles.js', PROJECT_ROOT),
  'utf8'
);
const shipSource = readFileSync(
  new URL('src/entities/Neon_Autopilot_V23_HighSpeed_DroneHeat.ship.js', PROJECT_ROOT),
  'utf8'
);
...
const { MESSAGES: I18N_MESSAGES } = require(fileURLToPath(
  new URL('src/ui/Neon_Autopilot_V23_HighSpeed_DroneHeat.i18n.js', PROJECT_ROOT)
));
...
test('main runtime uses categorized startup and safe storage boundaries', () => {
  assert.match(source, /NeonV23DependencyError/);
  assert.match(source, /NeonV23RenderingError/);
...
test('idle creep fails closed at startup and remains independently observable through HUD and diagnostics', () => {
  assert.match(
    source,
    /const idleCreepBootstrapContract = window\.NeonV23GameplayCore\s*\?\.IDLE_CREEP_CONTRACT;/
  );
  ...
  assert.match(dependency, /'V23 first-gear idle-creep contract'/);
...
test('runtime language changes re-project every dynamic surface without mutating gameplay authority', () => {
  assert.match(source, /const i18n = requireDependency\(window\.NeonV23I18n, 'V23 i18n module'\)/);
...
test('classified candlelight and obstacle factories preserve gameplay authority and pooled root contracts', () => {
  assert.match(source, /requireDependency\(\s*window\.NeonV23Candlelight,\s*'V23 candlelight presentation module'/);
  assert.match(source, /requireDependency\(\s*window\.NeonV23Obstacles,\s*'V23 obstacle presentation module'/);
  ...
  assert.match(
    source,
    /NeonV23World\.createDecorationLayer\(\{[\s\S]*?visualSeed: String\(visualSeed\),[\s\S]*?isReducedMotionEnabled/
  );
  ...
  assert.match(source, /const usesClassifiedObstacleVisual = obj\.mesh\.userData\.neonV23Obstacle !== undefined/);
...
test('jump-platform landing authority pins the target during preflight and both physical roads in flight', () => {
  ...
  const trackVisualUpdate = sourceBetween(
    ...
    '\n\n  requireDependency(window.NeonV23Ship'
  );
...
test('one physical-lighting rig owns HDR rendering, real shadows, and reachable covered-route fixtures', () => {
  assert.match(source, /requireDependency\(window\.NeonV23Lighting, 'V23 physical lighting module'\)/);
  assert.match(source, /const lightingBtn = document\.getElementById\('lightingBtn'\)/);
  assert.match(source, /requireDependency\(lightingBtn, 'V23 lighting toggle'\)/);
  ...
  assert.match(source, /lightingPlayerLocalLightCount: window\.NeonV23Ship\.SKY_CAPE_ART_CONTRACT\.roadProjectingLocalLightCount/);
...
test('visible FPS reuses the one-second renderer audit and publishes a critical tier without another frame loop', () => {
  ...
  assert.match(source, /requireDependency\(fpsDisplayEl, 'V23 FPS display'\)/);
...
test('speedometer uses a reviewable road, target, and overspeed scale without capping digital output', () => {
  ...
  assert.match(source, /requireDependency\([\s\S]*?'V23 speedometer'/);
...
test('themed music consumes presentation state only and shares one trusted master-toggle boundary', () => {
  assert.match(source, /requireDependency\(window\.NeonV23MusicLibrary, 'V23 music library module'\)/);
  assert.match(source, /requireDependency\(window\.NeonV23MusicPlayer, 'V23 music player module'\)/);
  assert.match(
    source,
    /const modelDebugPreferenceStorage = launchOptions\.modelDebug[\s\S]*?getItem\(key\) \{ return startup\.readString\(key, ''\); \}[\s\S]*?setItem\(\) \{\}[\s\S]*?NeonV23Audio\.createController\([\s\S]*?storage: modelDebugPreferenceStorage[\s\S]*?NeonV23MusicPlayer\.createController\([\s\S]*?storage: modelDebugPreferenceStorage/,
...
test('public diagnostics exclude live state and debug hooks stay narrow', () => {
  ...
  assert.match(source, /window\.NeonV23Diagnostics = Object\.freeze\(publicDiagnostics\)/);
  assert.doesNotMatch(source, /NeonV23Diagnostics\.state/);
});
...
test('minimap recoverable failures retry twice, escalate the third consecutive frame, and reset after success', () => {
  ...
  const recovery = new Function(
    ...
    { NeonV23MinimapErrors: { isRecoverable: (error) => error?.recoverable === true } },
    diagnostics,
    3,
    (fatalError, options) => {
...
test('minimap presentation owners dispose once and debug fault injection stays model-debug-only', () => {
  ...
  const debugSource = sourceBetween(
    ...
    'window.NeonV23Diagnostics = Object.freeze(publicDiagnostics)'
  );
  assert.match(
    debugSource,
...
test('spawn success counters are owned by their timer channels', () => {
  ...
  const pickupSpawner = sourceBetween('function spawnPickup(', "requireDependency(window.NeonV23World");
...
test('hood collision guide projects the authoritative player collider only during active hood flight', () => {
  ...
  assert.match(
    source,
    /requireDependency\(\s*hoodCollisionGuideEl[\s\S]*?'V23 hood collision guide'\s*\)/
  );
...
test('sky and ground consume one route-realm state for every scene transition', () => {
  ...
  assert.match(worldSource, /const configuredWorld = window\.NeonV23Config\?\.world/);
...
test('one weather snapshot drives visual atmosphere and bounded runtime-owned surface response', () => {
  ...
  assert.match(source, /requireDependency\(window\.NeonV23Weather, 'V23 weather module'\)/);
  ...
  assert.match(worldSource, /V23\.Weather\.Rain/);
  ...
  assert.match(
    source,
    /const surfaceWeatherModule = requireDependency\([\s\S]*?window\.NeonV23SurfaceWeather[\s\S]*?'V23 surface weather module'\s*\)/
  );
