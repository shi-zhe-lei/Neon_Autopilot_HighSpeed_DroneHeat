...
const FIXTURE_PATH = join(
  PROJECT_ROOT,
  'tests/browser/Neon_Autopilot_V23_HighSpeed_DroneHeat.gameover-layout-test.html'
);
...
test('modelDebug exposes one narrow game-over presentation hook backed only by endGame', () => {
  ...
  const publicSealIndex = runtime.indexOf('window.NeonV23Diagnostics = Object.freeze(publicDiagnostics)', hookIndex);
...
test('standalone game-over fixture owns the complete phone and iPad orientation matrix', () => {
  ...
  assert.match(fixture, /const fixtureVersion = '23-ipad-road-residency-253';/);
...
test('fixture starts the real run before requesting the real game-over boundary', () => {
  ...
  const hookLookupIndex = fixture.indexOf(
    'win.NeonV23Diagnostics?.testHooks?.presentGameOverForLayoutAudit',
  ...
  assert.doesNotMatch(fixture, /NeonV23Diagnostics(?:\?|)\.state/);
