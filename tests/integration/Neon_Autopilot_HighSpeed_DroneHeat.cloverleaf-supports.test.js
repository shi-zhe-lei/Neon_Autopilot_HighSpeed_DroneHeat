...
const cloverleafVisuals = globalThis.NeonCloverleafVisuals;
const cloverleafTilePool = globalThis.NeonCloverleafTilePool;
const productionTrack = globalThis.NeonTrack;
...
function assertDeclaredLocalEnvelope(geometry, tolerance = 0.000_001) {
  ...
  const declared = geometry.userData.neonSkyRouteScenery?.localEnvelope;
      ...
      for (const axis of [0, 1, 2]) {
        for (const side of [-1, 1]) {
          ...
          assert.ok(
            ...
            `${geometry.userData.neonSkyRouteScenery.role}: axis ${axis}/${side} faces inward`
          );
        }
      }
    }
...
test('Sky route scenery v2 freezes wind-stone topology, unit envelopes, matte finishes, and constant candle-stars', () => {
  ...
  try {
    ...
    for (const { mesh, role, spec, expectedVertexCount } of geometryCases) {
      const metadata = mesh.geometry.userData.neonSkyRouteScenery;
    ...
    const tunnelMetadata = visual.tunnelShells.geometry.userData.neonSkyRouteScenery;
    ...
    for (const renderQualityId of ['low', 'medium', 'high', 'medium']) {
      ...
      for (const mesh of structuralMeshes) {
        ...
        const metadata = material.userData.neonSkyRouteScenery;
    ...
    assert.equal(
      visual.underpassLights.material.userData.neon.kind,
...
test('ambient companions and route wayfinding use sealed Sky forms without changing traffic or decision authority', () => {
  ...
  try {
    ...
    const trafficArt = trafficGeometry.userData.neonAmbientTrafficArt;
    ...
    assert.equal(traffic.userData.neon.family, trafficContract.productionFamily);
    ...
    const goreArt = goreGeometry.userData.neonSkyWayfindingArt;
    ...
    assert.equal(
      decisionGeometry.userData.neonSkyWayfindingArt.role,
      wayfindingContract.decisionStructure.role
    );
    assert.equal(
      decisionGeometry.userData.neonSkyWayfindingArt.topology,
    ...
    assert.equal(
      visual.underpassLights.userData.neonSkyWayfindingArt.role,
    ...
    assert.equal(
      visual.exitSigns.userData.neonSkyWayfindingArt.style,
      wayfindingContract.exitSign.style
    );
    assert.equal(
      visual.exitSigns.material.userData.neonSkyWayfindingArt.textAuthority,
...
test('exit-sign atlas swaps localized text without rebuilding route geometry', () => {
  ...
  assert.match(signSource, /window\.NeonI18n\.subscribe\(syncLanguage\)/);
...
test('resident tunnel road, shell, wall, ceiling, and rib irradiance stay in existing draw budgets', () => {
  ...
  try {
    ...
    for (const renderQualityId of ['low', 'medium', 'high', 'medium']) {
      ...
      assert.equal(visual.roadShell.material.userData.neonStaticTunnelIrradiance, 2);
      assert.equal(visual.tunnelShells.material.emissive.getHex(), 0);
      assert.equal(visual.tunnelShells.material.userData.neonStaticTunnelIrradiance, 2);
      assert.equal(visual.beams.material.emissive.getHex(), 0);
      assert.equal(visual.beams.material.userData.neonStaticTunnelIrradiance, 2);
    }
...
test('Low, Medium, and High road PBR stay prebuilt, reversible, and outside route authority', () => {
  for (const marker of [
    ...
    'neon-standard-road-coordinate-v4',
    'neon-low-road-static-tunnel-irradiance-v3',
    'neon-ultra-road-coordinate-v6',
    "low-physical-without-coordinate-shader",
    'neonRoadMicroLengthSquared',
    'step(1.0e-10, neonRoadMicroLengthSquared)',
    'inversesqrt(max(neonRoadMicroLengthSquared',
    'mainline: 0xa9_aaa5',
    ...
    '0.02375 * neonRoadDetailFade',
    'vec2(32.0, 1.0)',
    '0.054 * neonRoadDetailFade',
    'mix(neonRoadGeometricNormal, normal, neonRoadDetailFade)',
    'vec2(45.75, 1.75)',
    '0.03 * neonRoadDetailFade',
  ...
  assert.doesNotMatch(source, /= normalize\(cross\(neonRoadDx, neonRoadDy\)\)/);
  ...
  assert.doesNotMatch(ultraRoadShader, /roadCoord\.y \/ 18\.0|neonRoadJointMask \* 0\.24/);
