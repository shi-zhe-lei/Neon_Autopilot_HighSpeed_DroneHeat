...
require(path.join(PROJECT_ROOT, 'src/navigation/Neon_Autopilot_V23_HighSpeed_DroneHeat.cloverleaf-map.js'));
...
const tilePool = globalThis.NeonV23CloverleafTilePool;
const productionMapFactory = globalThis.NeonV23CloverleafMap;
...
function validateAtomicFramePresentation(graph) {
  ...
  check(injectedFailure?.name === 'NeonV23MinimapRecoverableError'
  ...
  diagnostics = minimap.getDiagnostics();
  check(presentationFailure?.name === 'NeonV23MinimapRecoverableError'
...
function validateResponsiveStyles() {
  const css = fs.readFileSync(
    path.join(PROJECT_ROOT, 'styles/Neon_Autopilot_V23_HighSpeed_DroneHeat.css'),
    'utf8'
  );
