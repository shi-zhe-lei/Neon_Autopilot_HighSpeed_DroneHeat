...
global.THREE = THREE;
require(path.join(root, 'src/rendering/Neon_Autopilot_HighSpeed_DroneHeat.modeling.js'));
require(path.join(root, 'src/navigation/Neon_Autopilot_HighSpeed_DroneHeat.track.js'));
require(path.join(root, 'src/navigation/Neon_Autopilot_HighSpeed_DroneHeat.cloverleaf.js'));
require(path.join(root, 'src/navigation/Neon_Autopilot_HighSpeed_DroneHeat.cloverleaf-tiles.js'));
const { createI18n } = require(path.join(root, 'src/ui/Neon_Autopilot_HighSpeed_DroneHeat.i18n.js'));
const track = global.NeonTrack;
const visualsFactory = global.NeonCloverleafVisuals;
const tilePoolFactory = global.NeonCloverleafTilePool;
...
test('real resident and atlas identity publication changes revision before GPU upload, while geometry and disposal do not', () => {
  ...
  const previousI18n = global.NeonI18n;
  const i18n = createI18n({}, { document: null, autoApply: false });
  global.NeonI18n = i18n;
  ...
  try {
    pool = tilePoolFactory.create({
      THREE, track, modeling: global.NeonModeling, scene,
    ...
    pool?.dispose();
    i18n.disconnect();
    if (previousI18n === undefined) delete global.NeonI18n;
    else global.NeonI18n = previousI18n;
    restoreDocument();
  }
});
