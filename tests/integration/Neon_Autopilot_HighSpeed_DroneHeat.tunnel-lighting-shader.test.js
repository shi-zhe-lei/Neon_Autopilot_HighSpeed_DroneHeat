...
function buildVisual(qualityProfileId, tunnelLightingUniforms = null) {
  ...
  if (tunnelLightingUniforms) scene.userData.neonTunnelLightingUniforms = tunnelLightingUniforms;
  const job = globalThis.NeonCloverleafVisuals.createBuildJob({
    THREE,
    scene,
    track: globalThis.NeonTrack,
    modeling: globalThis.NeonModeling,
...
function assertReceiverPipeline(shader) {
  ...
  assert.equal((fragment.match(/float neonTunnelOutdoorTransmission\(/g) || []).length, 1);
  ...
  assert.match(directionalLoop, /getDirectionalLightInfo[\s\S]*?neonTunnelOutdoorTransmission\(0\.025, neonGlobalDirectionalTransmission\)[\s\S]*?getShadow[\s\S]*?RE_Direct/);
  assert.equal(
    directionalLoop.replace(/\n\s*directLight\.color \*= neonTunnelOutdoorTransmission\(0\.025, neonGlobalDirectionalTransmission\);/, ''),
  ...
  for (const variable of ['irradiance', 'iblIrradiance', 'radiance', 'clearcoatRadiance']) {
    ...
    const attenuation = main.indexOf(`${variable} *= neonTunnelOutdoorTransmission(0.14, `);
...
function createDiffuseProbe(shader) {
  ...
  const warmMatch = shader.fragmentShader.match(
    /irradiance \+= PI \* vec3\(([^)]+)\)\s*\* max\(vNeonStaticTunnelIrradiance, 0\.0\) \/ ([\d.]+);/
  );
  assert.ok(warmMatch);
  ...
  const transmissionExpression = shader.fragmentShader.match(
    /float neonTunnelOutdoorTransmission\(float deepTransmission, float globalTransmission\)\s*\{\s*return ([^;]+);\s*\}/
  );
  ...
  const evaluateTransmission = new Function(
    'mix', 'clamp', 'min', 'max', 'vNeonTunnelEnclosure', 'deepTransmission', 'globalTransmission',
      ...
      try {
        ...
        for (const quality of ['low', 'medium', 'high', 'low', 'high', 'medium']) {
          ...
          if (quality === 'high') {
            ...
            assert.match(roadShader.fragmentShader, /neonRoadGrain/);
