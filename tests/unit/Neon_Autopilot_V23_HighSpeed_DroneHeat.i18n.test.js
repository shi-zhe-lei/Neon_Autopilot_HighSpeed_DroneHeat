...
function assertCatalogContract() {
  ...
  assert.equal(library.STORAGE_KEY, 'cc-v23-ui-language');
  ...
  assert.ok(Object.keys(chinese).length >= 600, 'The full V23 UI catalog must remain covered');
...
function assertPresentationMetadataCoverage() {
  ...
  const sources = [];
  for (const profile of Object.values(context.window.NeonV23Config.renderQualityProfiles)) {
    sources.push(profile.name, profile.description);
  }
  for (const zone of context.window.NeonV23Config.zones) sources.push(zone.name);
  for (const mode of Object.values(context.window.NeonV23Weather.weatherModes)) {
    sources.push(mode.name, mode.description);
  }
  for (const type of Object.values(context.window.NeonV23Weather.types)) sources.push(type.name);
  for (const mode of Object.values(context.window.NeonV23MusicLibrary.PLAY_MODES)) {
    sources.push(mode.name, mode.description);
  }
  for (const track of context.window.NeonV23MusicLibrary.TRACKS) {
...
function assertShippedMarkupCoverage() {
  ...
  assert.doesNotMatch(source, /\blocalStorage\b/u, 'Storage must stay behind NeonV23Startup');
