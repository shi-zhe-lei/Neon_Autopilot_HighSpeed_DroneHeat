...
const candlelight = global.window.NeonV23Candlelight;
const modeling = global.window.NeonV23Modeling;
...
test('validates config mappings when supplied and stays usable without config', () => {
  ...
  assert.equal(candlelight.validateZones(global.window.NeonV23Config.zones), true);
  const copiedZones = global.window.NeonV23Config.zones.map((zone) => ({
    id: zone.id,
...
test('prewarms all assets and acquires without geometry or material allocation misses', () => {
  ...
  for (const object of [...acquired, ...ambientClusters]) {
    ...
    object.traverse((node) => {
      if (node.isMesh) {
        assert.equal(
          node.geometry.userData.neonV23ImmutableTopology,
          true,
...
test('builds six dim instanced ambient clusters with no halo, trail, pickup, map, or collision role', () => {
  ...
  for (const definition of candlelight.AMBIENT_FAMILY_CATALOG) {
    ...
    assert.equal(cluster.getObjectByName('V23.Candlelight.HaloEffect'), undefined);
    assert.equal(cluster.getObjectByName('V23.Candlelight.TrailEffect'), undefined);
