...
require(path.join(PROJECT_ROOT, 'src/audio/Neon_Autopilot_V23_HighSpeed_DroneHeat.music-player.js'));

const playerModule = global.window.NeonV23MusicPlayer;
...
test('adaptive library stays allocation-free until trusted intent and crossfades realm tracks', async () => {
  ...
  assert.equal(playerModule.version, 'V23-music-player-8');
...
test('the library equalizer consumes live bands instead of a decorative timer', () => {
  const playerSource = fs.readFileSync(
    path.join(PROJECT_ROOT, 'src/audio/Neon_Autopilot_V23_HighSpeed_DroneHeat.music-player.js'),
    'utf8'
  );
