...
function loadModule() {
  ...
  vm.runInContext(modelingSource, context, {
    filename: 'Neon_Autopilot_V23_HighSpeed_DroneHeat.modeling.js'
  });
  vm.runInContext(source, context, {
    filename: 'Neon_Autopilot_V23_HighSpeed_DroneHeat.obstacles.js'
  });
  return { obstacles: context.window.NeonV23Obstacles, window };
}
...
function loadConfiguredZones() {
  ...
  return context.window.NeonV23Config.zones;
}
...
test('matches the current config taxonomy without importing config into the entity module', () => {
  ...
  assert.doesNotMatch(source, /NeonV23Config|src\/config|obstacleFamilies\s*=/);
...
test('requires dependency injection, rejects external state, and returns a direct Object3D root', () => {
  ...
  const factory = loaded.obstacles.createFactory({
    THREE,
    modeling: loaded.window.NeonV23Modeling,
  ...
  assert.equal(root.userData.neonV23Obstacle.publicRootTransform, 'translation-only');
  assert.equal(root.userData.neonV23Obstacle.blockingRootRotation, 'identity');
  assert.equal(root.userData.neonV23Obstacle.fogParticipation, false);
...
test('keeps instance materials deeply isolated while preserving intra-instance source sharing semantics', () => {
  ...
  for (let index = 0; index < firstMaterials.length; index++) {
    ...
    assert.notEqual(firstMaterials[index].userData.neonV23, secondMaterials[index].userData.neonV23);
  ...
  firstMaterials[2].userData.neonV23.mutated = true;
  assert.equal(secondMaterials[2].opacity, secondOpacity);
  assert.equal(secondMaterials[2].userData.neonV23.mutated, undefined);
...
test('uses deterministic independent visual seeds and allocates no frame assets after prewarm', () => {
  ...
  assert.equal(
    first.userData.neonV23Obstacle.visualSeedHash,
    second.userData.neonV23Obstacle.visualSeedHash
  );
