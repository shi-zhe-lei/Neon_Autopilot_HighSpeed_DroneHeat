...
require(path.join(PROJECT_ROOT, 'src/weather/Neon_Autopilot_HighSpeed_DroneHeat.weather.js'));
...
const weatherEntries = Object.entries(globalThis.NeonWeather.types);
...
test('the shipped r160 PMREM cache refreshes enclosure HDR once per signature and releases every replaced target', () => {
  ...
  for (const quality of ['low', 'medium', 'high']) {
    ...
    const shared = { zoneIndex: 3, weather: globalThis.NeonWeather.types.clear.profile, dtSeconds: 1 / 60 };
...
test('scene-scoped tunnel uniforms compensate the actual global lighting once and retain ownership through toggles', () => {
  ...
  const uniforms = scene.userData.neonTunnelLightingUniforms;
  const originalEntries = Object.values(uniforms);
  assert.notEqual(uniforms, second.scene.userData.neonTunnelLightingUniforms);
  assert.deepEqual(originalEntries.map((entry) => entry.value), [1, 1, 1]);
  const plainInput = {
    zoneIndex: 0,
    weather: globalThis.NeonWeather.types.clear.profile,
    covered: true,
    underpassBlend: 1,
  ...
  assert.deepEqual(Object.values(second.scene.userData.neonTunnelLightingUniforms).map((entry) => entry.value), [1, 1, 1]);
  for (const quality of ['low', 'medium', 'high']) {
    ...
    for (const [weatherId, weatherType] of weatherEntries) {
      ...
      assert.ok(Math.abs(uniforms.neonGlobalAmbientTransmission.value - expectedAmbient) < 0.000_001,
        `${quality}/${weatherId}: snow, lightning and floors must remain in the ambient ratio`);
      assert.ok(Math.abs(uniforms.neonGlobalDirectionalTransmission.value - expectedDirectional) < 0.000_001);
      assert.ok(Math.abs(uniforms.neonGlobalEnvironmentTransmission.value
        - enclosed.environmentIntensity / outdoor.environmentIntensity) < 0.000_001);
      assert.equal(scene.userData.neonTunnelLightingUniforms, uniforms);
  ...
  const cachedTransmission = uniforms.neonGlobalEnvironmentTransmission.value;
  ...
  assert.equal(uniforms.neonGlobalEnvironmentTransmission.value, cachedTransmission,
  ...
  assert.deepEqual(Object.values(uniforms).map((entry) => entry.value), [1, 1, 1]);
  rig.setEnabled(true);
  assert.ok(uniforms.neonGlobalDirectionalTransmission.value < 0.02);
  rig.dispose();
  assert.equal(Object.hasOwn(scene.userData, 'neonTunnelLightingUniforms'), false);
  second.rig.dispose();

  const previous = { callerOwned: true };
  const previousScene = new THREE.Scene();
  previousScene.userData.neonTunnelLightingUniforms = previous;
  ...
  assert.notEqual(previousScene.userData.neonTunnelLightingUniforms, previous);
  restoreRig.dispose();
  assert.equal(previousScene.userData.neonTunnelLightingUniforms, previous);
  const replacedRig = makeRig();
  const replacement = { replacementOwned: true };
  previousScene.userData.neonTunnelLightingUniforms = replacement;
  replacedRig.dispose();
  assert.equal(previousScene.userData.neonTunnelLightingUniforms, replacement);
});
...
test('forward tunnel preview prelights fixtures without changing outdoor celestial or HDR state', () => {
  const input = {
    zoneIndex: 2,
    weather: globalThis.NeonWeather.types.cloudy.profile,
    covered: false,
    underpassBlend: 0
  };
...
test('open-underpass fixtures require central shade and weak sunlight while keeping the sun dominant', () => {
  ...
  const darkBridgeEdge = deriveLightingState({
    zoneIndex: 2,
    weather: globalThis.NeonWeather.types.thunderstorm.profile,
    covered: false,
...
test('every render tier bakes enclosure intensity into HDR and refreshes the environment through a portal', () => {
  ...
  for (const renderQualityId of Object.keys(tierInputs)) {
    ...
    const sharedInput = {
      zoneIndex: 3,
      weather: globalThis.NeonWeather.types['heavy-rain'].profile,
      lightningPulse: 0
    };
...
test('player toggle disables physical lights, HDR work, and projected shadows, then restores current authored state', () => {
  ...
  const disabledState = rig.update({
    zoneIndex: 5,
    weather: globalThis.NeonWeather.types.sandstorm.profile,
    covered: true,
    underpassBlend: 1,
    tunnelKind: 'underground-tunnel'
  });
