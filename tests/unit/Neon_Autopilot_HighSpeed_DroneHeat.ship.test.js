...
require(path.join(PROJECT_ROOT, 'src/entities/Neon_Autopilot_HighSpeed_DroneHeat.ship.js'));

const shipModule = global.window.NeonShip;
...
const source = fs.readFileSync(
  path.join(PROJECT_ROOT, 'src/entities/Neon_Autopilot_HighSpeed_DroneHeat.ship.js'),
  'utf8'
);
assert.match(
  source,
...
const shipConstructionQuality = global.window.NeonModeling.resolveQualityProfile('high');
const shipState = {
  lives: 3,
  speed: 0,
...
for (const objectName of [
  'Neon.Ship.streamlined-fuselage',
  'Neon.Ship.sealed-belly-shell',
  'Neon.Ship.embedded-canopy',
  'Neon.Ship.nose-lens',
  'Neon.Ship.candle-soul-core',
  'Neon.Ship.four-point-dawn-mark',
  'Neon.Ship.cape-wing-left',
  'Neon.Ship.cape-inset-left',
  'Neon.Ship.cape-wing-right-healthy',
  'Neon.Ship.cape-inset-right-healthy',
  'Neon.Ship.cape-wing-right-wounded-sealed',
  'Neon.Ship.cape-inset-right-wounded-sealed',
  'Neon.Ship.cape-wing-right-critical-sealed',
  'Neon.Ship.cape-inset-right-critical-sealed',
  'Neon.Ship.spirit-scarf-tail-left',
  'Neon.Ship.spirit-scarf-tail-right',
  'Neon.Ship.main-engine-cowl',
  'Neon.Ship.sealed-main-nozzle',
  'Neon.Ship.side-pod-left',
  'Neon.Ship.side-pod-right',
  'Neon.ShipEffect.main-flame',
  'Neon.ShipEffect.inner-flame',
  'Neon.ShipEffect.side-flame-left',
  'Neon.ShipEffect.side-flame-right',
  'Neon.ShipEffect.side-inner-flame-left',
  'Neon.ShipEffect.side-inner-flame-right',
  'Neon.ShipEffect.brake-nozzle-left',
  'Neon.ShipEffect.brake-nozzle-right',
  'Neon.ShipEffect.brake-flare-left',
  'Neon.ShipEffect.brake-flare-right',
  'Neon.ShipEffect.brake-inner-flare-left',
  'Neon.ShipEffect.brake-inner-flare-right',
  'Neon.ShipEffect.spirit-wake-ribbon-1',
  'Neon.ShipEffect.spirit-wake-ribbon-2',
  'Neon.ShipEffect.spirit-wake-ribbon-3',
  'Neon.ShipEffect.spirit-wake-ribbon-4',
  'Neon.ShipEffect.speed-streaks',
  'Neon.ShipEffect.shield',
  'Neon.ShipEffect.shield-field',
  'Neon.ShipEffect.shield-anchor-array',
  'Neon.ShipEffect.top-view-halo',
  'Neon.Ship.DamageEffects',
  'Neon.ShipEffect.scorch-nose',
  'Neon.ShipEffect.canopy-crack-left',
  'Neon.ShipEffect.damage-spark-0'
]) {
...
const spiritEyes = allShipObjects.filter((object) => object.name.startsWith('Neon.ShipEffect.spirit-eye-'));
assert.equal(spiritEyes.length, capeContract.spiritMaskEyeCount);
const leftCapePleats = allShipObjects.filter((object) => object.name.startsWith('Neon.ShipEffect.cape-rib-left-'));
const healthyRightCapePleats = allShipObjects.filter((object) => (
  object.name === 'Neon.ShipEffect.cape-rib-right-healthy'
  || object.name.startsWith('Neon.ShipEffect.cape-rib-right-healthy-')
));
...
const shield = requireSceneObject('Neon.ShipEffect.shield');
const shieldField = requireSceneObject('Neon.ShipEffect.shield-field');
const shieldOrbits = [
  requireSceneObject('Neon.ShipEffect.shield-orbit-horizon'),
  requireSceneObject('Neon.ShipEffect.shield-orbit-meridian'),
  requireSceneObject('Neon.ShipEffect.shield-orbit-crown')
];
const shieldAnchors = requireSceneObject('Neon.ShipEffect.shield-anchor-array');
...
assert.notEqual(shieldField.userData.neonBloom, true, 'The broad field must not enter selective Bloom');
...
for (const effect of [shield, shieldField, ...shieldOrbits, shieldAnchors]) {
  assert.equal(effect.userData.neon.modelClass, 'effect');
  assert.equal(effect.userData.neon.collisionIndependent, true);
...
for (const emitter of [...shieldOrbits, shieldAnchors]) {
  assert.equal(emitter.userData.neonBloom, true, `${emitter.name} must be an explicit narrow Bloom emitter`);
  assert.equal(emitter.layers.isEnabled(12), true);
}
const shieldFieldTopology = global.window.NeonModeling.analyzeTopology(shieldField.geometry);
...
for (const name of ['Neon.Ship.tail-fin-left', 'Neon.Ship.tail-fin-right-healthy']) {
...
for (const [leftName, rightName] of [
  ['Neon.Ship.cape-wing-left', 'Neon.Ship.cape-wing-right-healthy'],
  ['Neon.Ship.cape-inset-left', 'Neon.Ship.cape-inset-right-healthy'],
  ['Neon.Ship.tail-fin-left', 'Neon.Ship.tail-fin-right-healthy'],
  ['Neon.Ship.spirit-scarf-tail-left', 'Neon.Ship.spirit-scarf-tail-right'],
  ['Neon.ShipEffect.cape-edge-left', 'Neon.ShipEffect.cape-edge-right-healthy']
]) {
  assertMirroredGeometryBounds(leftName, rightName);
}

const healthyRightEdge = requireSceneObject('Neon.ShipEffect.cape-edge-right-healthy');
assert.equal(healthyRightEdge.parent.name, 'Neon.ShipCape.right.healthy');
for (const [leftName, rightName] of [
  ['Neon.ShipEffect.hull-flow-left', 'Neon.ShipEffect.hull-flow-right'],
  ['Neon.ShipEffect.spirit-scarf-seam-left', 'Neon.ShipEffect.spirit-scarf-seam-right'],
  ['Neon.ShipEffect.cape-rim-left', 'Neon.ShipEffect.cape-rim-right-healthy'],
  ['Neon.ShipEffect.constellation-thread-left', 'Neon.ShipEffect.constellation-thread-right-healthy'],
  ['Neon.ShipEffect.cape-rib-left-forward', 'Neon.ShipEffect.cape-rib-right-healthy'],
  ['Neon.ShipEffect.cape-rib-left-aft', 'Neon.ShipEffect.cape-rib-right-healthy-aft'],
  ['Neon.ShipEffect.wingtip-light-left', 'Neon.ShipEffect.wingtip-light-right-healthy']
]) {
  assert.equal(
...
for (let index = 1; index <= 4; index++) {
  assert.equal(
    requireSceneObject(`Neon.ShipEffect.constellation-node-left-${index}`).userData.phase,
    requireSceneObject(`Neon.ShipEffect.constellation-node-right-healthy-${index}`).userData.phase
  );
}
...
const mainFlame = requireSceneObject('Neon.ShipEffect.main-flame');
const innerMainFlame = requireSceneObject('Neon.ShipEffect.inner-flame');
const leftSideFlame = requireSceneObject('Neon.ShipEffect.side-flame-left');
const rightSideFlame = requireSceneObject('Neon.ShipEffect.side-flame-right');
const forwardPlumeRoots = new Map([
  ...
  ['Neon.ShipEffect.side-inner-flame-left', componentClearanceContract.sidePlumeRootZM],
  ['Neon.ShipEffect.side-inner-flame-right', componentClearanceContract.sidePlumeRootZM]
]);
...
assert.ok(
  fullSteerPlumeSamples.get('Neon.ShipEffect.side-flame-left').maximumAxialM
    > fullSteerPlumeSamples.get('Neon.ShipEffect.side-flame-right').maximumAxialM,
  'Full-right steering must lengthen only the left outer tail from its fixed aperture'
);
assert.ok(
  fullSteerPlumeSamples.get('Neon.ShipEffect.side-inner-flame-left').maximumAxialM
    > fullSteerPlumeSamples.get('Neon.ShipEffect.side-inner-flame-right').maximumAxialM,
...
for (const side of ['left', 'right']) {
  const outerBrake = measurePlumeAxis(`Neon.ShipEffect.brake-flare-${side}`);
  const innerBrake = measurePlumeAxis(`Neon.ShipEffect.brake-inner-flare-${side}`);
...
const hull = requireSceneObject('Neon.Ship.streamlined-fuselage');
const leftCape = requireSceneObject('Neon.Ship.cape-wing-left');
const leftWingGlow = requireSceneObject('Neon.ShipEffect.cape-edge-left');
...
for (const material of originalOpaqueMaterials) {
  ...
  assert.match(shader.fragmentShader, /neonDiffusePattern/);
  assert.match(shader.fragmentShader, /neonRoughnessPattern/);
  const bounceLine = 'irradiance += PI * neonShipTunnelBounce * 0.72 / 0.18;';
  ...
  assert.doesNotMatch(shader.fragmentShader, /\b(?:totalEmissiveRadiance|radiance|directDiffuse|directSpecular)\s*\+=?[^;]*neonShipTunnelBounce/);
}
...
for (const material of [clonedBounceHull, secondGenerationBounceHull]) {
  ...
  assert.match(shader.fragmentShader, /neonDiffusePattern/);
  assert.match(shader.fragmentShader, /neonRoughnessPattern/);
  assert.equal(shader.fragmentShader.split('irradiance += PI * neonShipTunnelBounce').length - 1, 1);
...
assert.equal(requireSceneObject('Neon.ShipEffect.main-flame').userData.neonBloom, true);
assert.equal(requireSceneObject('Neon.ShipEffect.main-flame').layers.isEnabled(12), true);
assert.equal(requireSceneObject('Neon.Ship.candle-soul-core').userData.neonBloom, true);
assert.notEqual(createdShip.playerShadow.userData.neonBloom, true);
assert.notEqual(hull.userData.neonBloom, true);
...
assert.equal(shipScene.getObjectByName('Neon.WatertightShip'), undefined);

console.log('Neon ship propulsion regression passed');
