...
const startup = global.window.NeonV23Startup;
const errors = global.window.NeonV23Errors;
assert.ok(startup, 'Startup boundary did not publish its browser contract');
assert.ok(errors.NeonV23RenderingError.prototype instanceof errors.NeonV23StartupError);
...
startup.showError(new errors.NeonV23RenderingError('Error creating WebGL context'));
...
windowListeners.get('error')?.({
  target: {
    tagName: 'SCRIPT',
    src: 'file:///D:/Gary/src/runtime/Neon_Autopilot_V23_HighSpeed_DroneHeat.js'
  }
});
...
assert.deepEqual(delegatedFatalEvents[3]?.options, {
  ...
  filename: 'file:///D:/Gary/src/runtime/Neon_Autopilot_V23_HighSpeed_DroneHeat.js'
});
assert.equal(
  startup.getDiagnostics().fatalErrorCount,
  3,
