...
require(path.join(PROJECT_ROOT, 'src/audio/Neon_Autopilot_HighSpeed_DroneHeat.audio.js'));

const audioModule = global.window.NeonAudio;
const effectAssetModule = global.window.NeonEffectAssets;
assert.ok(audioModule, 'Audio module did not publish its browser contract');
assert.equal(effectAssetModule.version, 'Neon-effect-assets-1');
assert.equal(effectAssetModule.releaseGeneration, 'neon-ipad-road-residency-253');
assert.equal(audioModule.version, 'Neon-audio-13');
assert.equal(audioModule.EFFECT_ASSET_RELEASE_GENERATION, 'neon-ipad-road-residency-253');
...
const audioSource = fs.readFileSync(
  path.join(PROJECT_ROOT, 'src/audio/Neon_Autopilot_HighSpeed_DroneHeat.audio.js'),
  'utf8'
);
assert.match(
  audioSource,
...
(async () => {
  ...
  try {
    ...
    assert.ok(
      fetchedUrls.every((url) => url.endsWith('?v=neon-ipad-road-residency-253')),
