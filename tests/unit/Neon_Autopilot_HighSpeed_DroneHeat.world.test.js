...
const configuredZones = configContext.window.NeonConfig.zones;
const context = vm.createContext({ window: {} });
vm.runInContext(worldSource, context, {
  filename: 'Neon_Autopilot_HighSpeed_DroneHeat.world.js'
});
const world = context.window.NeonWorld;
...
test('fresh committed route shelter masks only local rain while open bridge exteriors remain visible', () => {
  ...
  assert.match(worldSource, /const weatherClassifier = window\.NeonWeather\?\.isSealedTunnelFrame;/);
  ...
  assert.match(worldSource, /uNeonRainLocalExposure/);
  assert.match(worldSource, /uNeonRainDistantExposure/);
  assert.match(worldSource, /length\(vNeonRainLocalXZ\)/);
  assert.match(worldSource, /customProgramCacheKey = \(\) => 'neon-local-rain-shelter-v1'/);
  assert.doesNotMatch(
    worldSource.slice(
...
test('High Ultra environment is reversible, uses configured far scenery, and owns no gameplay authority', () => {
  ...
  for (const marker of [
    ...
    'uNeonUltraWorldDetail',
    'uNeonUltraCloudDetail'
  ]) {
  ...
  assert.match(horizonBuildSource, /attribute vec3 neonNextHorizonColor;/);
  assert.match(horizonBuildSource, /uniform float neonHorizonColorBlend;/);
  assert.match(
    horizonBuildSource,
    /vColor\s*=\s*mix\(\s*vColor,\s*neonNextHorizonColor,\s*clamp\(neonHorizonColorBlend,\s*0\.0,\s*1\.0\)\s*\);/
  );
...
test('classified region and creature integration contracts freeze exact pools and distance gates', () => {
  ...
  for (const marker of [
    ...
    'Neon creature preview must share the configured world transition length and segmentation.',
    'Candlelight.validateZones(zones)',
    'Neon world visualSeed must be a string.',
...
test('real Three fixed-seed construction grounds every non-airborne root and publishes Film envelopes', () => {
  ...
  try {
    ...
    console.info = () => {};
    console.warn = () => {};
    layer = global.window.NeonWorld.createDecorationLayer({
      THREE,
      scene,
      state,
      roadCenterAt: () => 0,
    ...
    const moonContract = global.window.NeonWorld.MOON_LIGHTING_CONTRACT;
    const moonAnchor = scene.getObjectByName('Neon.Sky.MoonAnchor');
    const moonVisual = scene.getObjectByName('Neon.Sky.CandleMoonMemoryLayers');
    const moonDisc = scene.getObjectByName('Neon.Sky.CandleMoonDisc');
    const moonHalo = scene.getObjectByName('Neon.Sky.CandleMoonHalo');
    const moonStarPetals = scene.getObjectByName('Neon.Sky.MemoryStarPetals');
    const moonCloudVeils = scene.getObjectByName('Neon.Sky.MemoryCloudVeils');
    ...
    const baselineContract = global.window.NeonWorld.BASELINE_SCENERY_PRESENTATION_CONTRACT;
    ...
    for (const root of dormantLegacyScenery) {
      ...
      root.traverse((object) => {
        ...
        const role = object.userData.neon?.role;
        ...
        assert.notEqual(object.material?.userData?.neon?.kind, 'glass');
      });
    ...
    const ultraContract = global.window.NeonWorld.WORLD_ULTRA_PRESENTATION_CONTRACT;
    const ultraHorizonGroups = scene.children
      .filter(({ name }) => /^Neon\.World\.UltraHorizon\.\d+$/.test(name))
    ...
    for (const [zoneIndex, group] of ultraHorizonGroups.entries()) {
      ...
      const colorBlendUniform = mesh.material.userData.neonHorizonColorBlend;
      ...
      assert.strictEqual(
        webgl1Shader.uniforms.neonHorizonColorBlend,
        colorBlendUniform
      );
      assert.match(
      ...
      const metadata = mesh.geometry.userData.neonHorizon;
      assert.ok(metadata);
      ...
      ultraHorizonIdentities.push({
        ...
        nextColor: mesh.geometry.getAttribute('neonNextHorizonColor'),
        colorBlendUniform,
        webgl1Shader,
    ...
    assert.equal(
      activeHorizonIdentity.webgl1Shader.uniforms.neonHorizonColorBlend.value,
      state.skyBlend
    );
    ...
    assert.equal(
      restoredHorizonIdentity.webgl1Shader.uniforms.neonHorizonColorBlend.value,
      state.skyBlend
    );
    assert.ok(
    ...
    const terrainContract = global.window.NeonWorld.SCENERY_TERRAIN_CONTACT_CONTRACT;
