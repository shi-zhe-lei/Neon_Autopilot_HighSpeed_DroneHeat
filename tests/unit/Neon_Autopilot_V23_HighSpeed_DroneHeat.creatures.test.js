...
const creatures = global.window.NeonV23Creatures;
...
test('prewarms unique closed finite nondegenerate geometry within every quality budget', () => {
  ...
  for (const root of factory.getTopologyRoots()) {
    ...
    assert.equal(high.geometry.userData.neonV23.spiritMaskCount, definition.geometry.clusterCount);
    assert.equal(high.geometry.userData.neonV23.maskEyeCount, definition.geometry.clusterCount * 2);
