...
const gameplaySource = readFileSync(
  new URL('src/gameplay/Neon_Autopilot_V23_HighSpeed_DroneHeat.gameplay-core.js', PROJECT_ROOT),
  'utf8'
);
...
vm.runInContext(gameplaySource, context, {
  filename: 'Neon_Autopilot_V23_HighSpeed_DroneHeat.gameplay-core.js'
});
vm.runInContext(source, context, {
  filename: 'Neon_Autopilot_V23_HighSpeed_DroneHeat.surface-weather.js'
});
const {
  ...
  stepSurfaceCell,
  stepSurfaceResponse
} = context.window.NeonV23SurfaceWeather;
const { sweptEllipseInterval } = context.window.NeonV23GameplayCore;
...
test('surface sheets stay within millimetres of the road and snow banks have a sealed flat base', () => {
  ...
  for (const [radialSegments, heightSegments] of [[8, 4], [12, 6]]) {
    ...
    assert.equal(geometry.userData.neonV23.flatRoadBase, true);
    geometry.dispose();
  }
  assert.match(
    source,
...
test('water and snow are laid out into a stable long horizon before the ship reaches them', () => {
  ...
  assert.equal(SURFACE_LAYOUT_CONTRACT.version, 'V23-surface-layout-4');
