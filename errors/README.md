...
`minimap.js` 定义 `NeonV23MinimapRecoverableError`，并用 `code`、`phase` 与 `recoverable=true` 区分仅影响一帧的缓冲分配、绘制和最终复制故障。路线、场景坐标或版本合同产生的 `TypeError` 不得包装成可恢复错误。运行时保留上一张完整地图并在下一 RAF 重试；一次成功会清零连续失败计数，连续第三次失败必须交给既有致命边界，不能静默吞错或无限循环。
